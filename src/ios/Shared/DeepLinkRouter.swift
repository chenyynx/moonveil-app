...
extension Notification.Name {
    /// Posted by `DeepLinkRouter` when `moonveil://open?session=…&path=…` lands
...
/// Parses `moonveil://` URLs into navigation actions and dispatches them
...
///   moonveil://share
///   moonveil://views/alarm
///   moonveil://open_terminal[?init_command=…]
///   moonveil://open?session=<sid>&path=<scope-prefixed-path>  (openminis.app launcher round-trip)
///   moonveil://session/<id>      (legacy singular alias)
///   moonveil://sessions/<id>     (canonical — matches minis-sessions-cli)
///   moonveil://settings
///   moonveil://settings/providers[/<instanceId>]
...
///   moonveil://settings/shared-folders             (alias: shared_folders)
///   moonveil://settings/logs
///   moonveil://settings/appearance
///   moonveil://settings/background
///   moonveil://settings/about
///   moonveil://settings/permissions
///   moonveil://settings/environments[?create_key=…&create_value=…&create_note=…]
///   moonveil://settings/rootfs                     (alias: mirrors, rootfs-management, rootfs_management)
///
...
enum DeepLinkRouter {
    ...
    static func handle(url: URL, shareCoordinator: ShareCoordinator) {
        guard url.scheme == "moonveil", let host = url.host else {
    ...
    private static func handleSettings(url: URL, coord: DeepLinkCoordinator) {
        ...
        // screen exists — so a `moonveil://settings/soul` link (or the agent
        // generating one) quietly under-delivered.
        case "soul":
            coord.pendingSettingsTarget = .soul

        // [T-mcp-oauth-deeplink] moonveil://settings/mcp-servers/<serverId> —
        // jump straight to the server's edit form (Authorize button). The
        // AUTH_REQUIRED error from moonveil-mcp-cli embeds this link. Server
    ...
    /// Parses a `moonveil://open?session=…&path=…` URL fired by the
    ...
    private static func handleWebAppLauncherReturn(url: URL) {
        ...
        guard let rawPath = components?.queryItems?.first(where: { $0.name == "path" })?.value,
            ...
            deepLinkLog.warning("moonveil://open missing path")
            return
        }
        ...
        if rawPath.hasPrefix("attachments/") {
            guard let sid = session, !sid.isEmpty else {
                deepLinkLog.warning("moonveil://open path=attachments/… missing session")
                return
            }
            ...
            guard let sid = session, !sid.isEmpty else {
                deepLinkLog.warning("moonveil://open path=workspace/… missing session")
                return
            }
            ...
            guard let slash = rest.firstIndex(of: "/") else {
                deepLinkLog.warning("moonveil://open path=mount:… missing /<rest>")
                return
            }
            scope = .mount
            ...
            deepLinkLog.warning("moonveil://open unknown path prefix in \(rawPath)")
            return
        }
        ...
        deepLinkLog.info("moonveil://open scope=\(scope.rawValue) ctx=\(ctx ?? "nil") htmlPath=\(htmlPath)")
