    ...
    private static func mixedScenario() -> [ParsedMessage] {
        [
            ...
            ParsedMessage(role: .assistant, blocks: [
                ...
                ParsedBlock(kind: .shellTool(command: "xcodebuild test -scheme Moonveil -destination 'platform=iOS Simulator,name=iPhone 15'"), content: "xcodebuild test ...", toolStatus: .success),
                tb("""
                All tests passed:
    ...
    private static func toolHeavyScenario() -> [ParsedMessage] {
        [
            ...
            ParsedMessage(role: .assistant, blocks: [
                ...
                ParsedBlock(kind: .shellTool(command: "security find-generic-password -s 'com.moonveil.auth'"), content: "security find-generic-password", toolStatus: .failed(message: "The specified item could not be found in the keychain.")),
                tb("Expected — no keychain entry exists yet. Let me set up the test environment:"),
                ParsedBlock(kind: .shellTool(command: "xcodebuild test -scheme Moonveil -only-testing:AuthTests"), content: "xcodebuild test", toolStatus: .failed(message: "xcodebuild: error: Testing failed.")),
                ...
                ParsedBlock(kind: .shellTool(command: "xcodebuild test -scheme Moonveil -only-testing:AuthTests"), content: "xcodebuild test", toolStatus: .success),
                tb("""
