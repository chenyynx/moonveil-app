...
/// e.g. "Moonveil analyze sleep" (or the same phrase pattern in the user's locale).
...
struct QuickTaskIntent: AppIntent {
    ...
    static var description = IntentDescription("Runs a predefined Moonveil task like sleep analysis, weather check, or morning briefing.")
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<SendPromptResult> & ProvidesDialog {
        ...
        if sendCompletionNotification {
            ShortcutNotification.post(
                ...
                title: AppLocalized("Moonveil: \(taskName)"),
        ...
        if waitForResult {
            ...
            if sendCompletionNotification {
                ShortcutNotification.post(
                    ...
                    title: AppLocalized("Moonveil: \(taskName) Done"),
        ...
        Task { @MainActor in
            ...
            if capturedSendCompletionNotification {
                ShortcutNotification.post(
                    ...
                    title: AppLocalized("Moonveil: \(capturedTaskName) Done"),
