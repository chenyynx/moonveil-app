...
final class BrowserUseManager: NSObject, ObservableObject {
    ...
    /// Shared scheme handler for `moonveil://` URLs across all browser tabs
    ...
    private static func makeWebView(profile: UserAgentProfile,
        ...
        config.setURLSchemeHandler(Self.sharedMinisSchemeHandler, forURLScheme: "moonveil")

        // Bridge JS window.print() to the native iOS print dialog. Injected on
        // every page (moonveil:// and external http/https) at document start so a
    ...
    private func navigate(to urlString: String?) async throws -> BrowserActionResult {
        ...
        // Only allow HTTP(S) and moonveil:// schemes for direct navigation
        let scheme = url.scheme?.lowercased() ?? ""
        if !["http", "https", "moonveil"].contains(scheme) {
            return .error("Cannot navigate to \(scheme):// URLs. Only http://, https://, and moonveil:// are supported.")
        }
...
extension BrowserUseManager: WKNavigationDelegate {
    ...
    nonisolated func webView(
        ...
        let allowedSchemes: Set<String> = ["http", "https", "about", "blob", "moonveil"]
            ...
            Task { @MainActor [weak self] in
                ...
                if let cont = self.navigationContinuation {
                    ...
                    cont.resume(throwing: URLError(.unsupportedURL, userInfo: [
                        NSLocalizedDescriptionKey: "Blocked navigation to \(scheme):// — only http(s) and moonveil:// are allowed in the browser."
                    ]))
                }
            }
            return
...
/// Handles `moonveil://` URLs in WKWebView by resolving them to local files.
/// Supports `moonveil://workspace/file.html`, `moonveil://shared/...`, etc.
...
final class BrowserDownloadCenter: ObservableObject {
    ...
    func began(sessionId: String, filename: String, progress: Progress,
        ...
        queueAgentEvent(sessionId: sessionId, filename: filename,
                        line: "\(filename): download started → saving to moonveil://workspace/\(filename)")
        return id
    }
    ...
    func finished(id: UUID, sizeText: String) {
        ...
        queueAgentEvent(sessionId: e.sessionId, filename: e.filename,
                        line: "\(e.filename): download COMPLETED (\(sizeText)) → moonveil://workspace/\(e.filename) (shell path: /var/minis/workspace/\(e.filename))")
    }
    ...
    func agentReport(for sessionId: String) -> String? {
        ...
        var lines: [String] = inflight.map { e in
            ...
            return "\(e.filename): downloading \(pct) (\(done) / \(total)) → saving to moonveil://workspace/\(e.filename)"
        }
