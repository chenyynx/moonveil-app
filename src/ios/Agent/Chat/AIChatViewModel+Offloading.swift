...
extension AIChatViewModel {
    ...
    /// Decide whether a file path (Linux view) or moonveil:// URL is backed by
    ...
    private static func isPersistentMinisPath(_ path: String) -> Bool {
        ...
        // moonveil:// URL view maps 1:1 to the directories above
        if path.hasPrefix("moonveil://workspace/") { return true }
        if path.hasPrefix("moonveil://browser/") { return true }
        if path.hasPrefix("moonveil://attachments/") { return true }
        if path.hasPrefix("moonveil://offloads/") { return true }
        return false
    }
    ...
    private static func originalImagePath(
        ...
        // browser_use (any action) appends `minis_url: moonveil://...` to the
        ...
        if toolName == "browser_use" || toolName.isEmpty {
            if let urlRange = content.range(of: #"moonveil://\S+"#, options: .regularExpression) {
    ...
    /// Returns a `moonveil://attachments/<filename>` URL string, or nil on failure.
    func saveAttachment(_ data: Data, filename: String) -> String? {
        ...
        return "moonveil://attachments/\(encoded)"
    }
