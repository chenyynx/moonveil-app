...
final class AIChatViewModel: ObservableObject, SpeechControlling {
    ...
    /// Matches: ![...](moonveil://attachments/file.mp3?auto_play=true)
    ...
    static func triggerAutoPlayAudio(in text: String) {
        ...
        // Resolve moonveil:// to file URL
        let fileURL: URL?
        if cleanURL.scheme == "moonveil" {
    ...
    /// Resolve moonveil:// URL to local file URL (standalone, no UI dependency).
    ...
    private var baseSystemPrompt: String {
        ...
        SystemPromptBuilder.identitySection()
            ...
            + "IMPORTANT: moonveil:// URLs are app-internal — they are NOT web URLs. Do NOT pass moonveil:// action URLs (open_terminal, views, settings) to browser_use — those are app deep links, use Markdown links in chat instead. "
            + "However, moonveil:// resource URLs CAN be opened in browser_use with navigate. All directories under /var/minis/ are accessible: workspace, attachments, offloads, shared, etc. "
            + "The built-in browser fully supports moonveil:// — HTML pages and all sub-resources (JS, CSS, images, fonts, etc.) referenced via moonveil:// absolute URLs or relative paths resolve correctly within the current session. "
            ...
            + "The browser resolves relative paths against the moonveil:// base URL automatically. "
            + "Cross-directory references also work with absolute moonveil:// URLs (e.g. <img src=\"moonveil://attachments/photo.png\"> from a workspace HTML page). "
            + "Navigate to the entry HTML to preview, e.g. moonveil://workspace/myapp/index.html.\n"
            ...
            + "IMPORTANT: moonveil:// URLs MUST be percent-encoded. Non-ASCII characters (Chinese, emoji, spaces, etc.) in filenames will break Markdown rendering if not encoded. "
            + "Use the minis_url from tool results directly — it is already encoded. "
            + "If you construct a moonveil:// URL manually, percent-encode the filename (e.g. %E4%B8%AD%E6%96%87 for non-ASCII characters).\n"
            ...
            + "Audio auto-play: append ?auto_play=true to an audio moonveil:// URL to auto-play when rendered (e.g. ![audio](moonveil://attachments/song.mp3?auto_play=true)). Use sparingly — only when the user explicitly asks to hear audio immediately.\n"
            ...
            + "Use Markdown links for all moonveil:// files — the user can tap to preview them directly in chat.\n\n"
            ...
            + "apple-alarm sets alarms and timers via AlarmKit (iOS 26+). Alarms can only be viewed in the Moonveil home screen (alarm icon in the top-right toolbar) or by opening moonveil://views/alarm. "
            + "After setting an alarm, tell the user it is visible on the Moonveil home screen and they can tap the alarm icon or open moonveil://views/alarm to manage it.\n"
            ...
            + "moonveil-open <url-or-path>: Opens a resource inside Moonveil without leaving the chat. Accepts http/https URLs (→ built-in WebKit preview) and chat-resource file paths under /var/minis/** (→ built-in file preview, routed by extension: images to the image viewer, .md to markdown preview, .html to HTML preview, .pdf/office docs to QuickLook, audio/video to the media player, else share sheet). Examples: `moonveil-open https://example.com`, `moonveil-open /var/minis/workspace/report.md`, `moonveil-open /var/minis/attachments/chart.png`. Prefer this over `apple-open` for anything that can be previewed in-app so the user doesn't lose conversation context. Use `apple-open` for non-web schemes (tel:, mailto:, maps://, settings, etc.) or when the user explicitly wants the system handler.\n"
            ...
            + "moonveil-model-use: Invoke other LLM models pre-configured by the user. "
            + "You have \(ProviderConfigStore.shared.resolvedAgentLoopEntries.count) model(s) available. "
            + "Use `moonveil-model-use list` to see them (includes each model's modality capabilities like image_output, audio_output, etc.), "
            ...
            + "Interactive terminal: moonveil://open_terminal opens a terminal for tasks that require interactive stdin (passwords, ssh, TUI apps like htop/vi). "
            ...
            + "[Set ENV_NAME](moonveil://settings/environments?create_key=ENV_NAME&create_value=&create_note=Used%20by%20XYZ) — "
    ...
    /// Currently active session ID — accessible from anywhere for moonveil:// URL resolution.
