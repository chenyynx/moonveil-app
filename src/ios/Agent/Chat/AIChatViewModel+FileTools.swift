...
extension AIChatViewModel {
    ...
    /// Resolve a moonveil:// URL string to a host filesystem URL.
    ...
    private func resolveMinisURL(_ urlString: String) -> URL? {
        guard let url = URL(string: urlString), url.scheme == "moonveil",
    ...
    /// Convert a Linux path under /var/minis/ to a moonveil:// URL, or nil if not under /var/minis/.
    ...
    /// Resolve a moonveil:// URL or /var/minis/ Linux path for direct reads.
    /// Unifies both URL schemes into a single async resolution path.
    func resolveMinisPath(_ pathOrURL: String) async -> URL? {
        if pathOrURL.hasPrefix("moonveil://") {
            // Convert moonveil:// URL to Linux path, then resolve via mount table
