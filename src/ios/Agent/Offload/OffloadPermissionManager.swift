...
final class OffloadPermissionManager: ObservableObject {
    ...
    func checkPermission(for command: String, sessionId: String?, fullCommand: String = "") async -> PermissionResult {
        ...
        case .notAllowed:
            ...
            return .denied("Permission denied: the user has disabled '\(command)'. To enable it, go to Settings > Permissions or tap: [Open Permissions](moonveil://settings/permissions)")
        ...
        case .askOnce:
            ...
            if allowed {
                ...
                if sessionGrants[sessionId]?.contains(command) == true {
                    ...
                    return .denied("Permission denied: authorization for '\(command)' timed out. To change permissions: [Open Permissions](moonveil://settings/permissions)")
                }
                return .denied("Permission denied: the user declined '\(command)' for this session. To change permissions: [Open Permissions](moonveil://settings/permissions)")
            }
        }
    }
