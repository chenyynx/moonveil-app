...
//  /var/minis/mcp-servers — the SAME file the in-guest `moonveil-mcp-cli`
...
struct MCPServerConfig: Codable, Identifiable, Hashable {
    ...
    /// lives entirely in the in-guest `moonveil-mcp-cli` daemon; the native layer
...
final class MCPStore: ObservableObject {
    ...
    // MARK: - servers.json (co-read/write with moonveil-mcp-cli)
    ...
    private struct ServerEntry: Codable {
        ...
        var args: [String]?
        var env: [String: String]?
        var startupTimeoutSeconds: Int?   // Moonveil STDIO startup timeout; round-tripped verbatim
    }
    ...
    func systemPromptSnippet(for sessionId: String) -> String? {
        ...
        var lines = "Available MCP Servers (use moonveil-mcp-cli to discover and call):\n"
        for s in selected {
            var note = s.note ?? ""
        ...
        lines += "\nTo use: run `moonveil-mcp-cli tools <server>` to see available tools,\n"
        lines += "then `moonveil-mcp-cli call <server> <tool> [args]` to invoke."
        // Agent-facing guidance on the runtime env placeholder; English-only,
        // not localized (this is prompt text, never shown in the UI).
        lines += "\nWhen adding or modifying an MCP server config (via moonveil-mcp-cli add / the UI), use $$VARNAME in env/headers/url values as a placeholder resolved at runtime from the system/App environment variables — do not hardcode secrets; reference an existing App environment variable as $$NAME."
        return lines
    }

    // MARK: - Tools refresh (native → in-guest CLI) [T-mcp-tools-refresh]

    /// One tool row from `moonveil-mcp-cli tools --refresh`.
    ...
    /// (`moonveil-mcp-cli refresh <name>`), so tools the remote added after the
    ...
    func refreshTools(server: String) async throws -> [MCPToolInfo] {
        ...
        let result = try await ISHExecutionCoordinator.shared.execute(
            ...
            command: "moonveil-mcp-cli refresh \(quoted)",
            timeout: 120,
            lineCallback: { _ in },
            pidCallback: { _ in }
        )
    ...
    /// Detect edits that bypassed the CRUD paths (in-guest moonveil-mcp-cli
