...
struct CloudSyncSettingsView: View {
    ...
    var body: some View {
        ...
        .alert("Are you absolutely sure?", isPresented: $showDeleteCloudStep2) {
            ...
            Text("This action cannot be undone. All Moonveil data stored in iCloud will be erased immediately. Other devices signed into the same iCloud account will lose their synced copies on the next sync.\n\nLocal data on this device is safe.")
        }
        .alert("iCloud Data Deleted", isPresented: $showDeleteCloudDone) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("All Moonveil data has been removed from iCloud. This device is now re-uploading its local content to a fresh iCloud zone.")
        }
