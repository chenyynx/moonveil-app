...
final class WebViewHolder: NSObject, ObservableObject {
    ...
    init(url: URL, localFile: Bool) {
        ...
        config.setURLSchemeHandler(BrowserUseManager.sharedMinisSchemeHandler, forURLScheme: "moonveil")

        // Bridge JS window.print() to the native print dialog, matching
        // BrowserUseManager so moonveil:// HTML opened from chat (which routes
    ...
    /// inside a `.fullScreenCover` (the iSH terminal → `moonveil-open` flow
...
struct WebPreviewMoreMenu: View {
    ...
    /// Only the moonveil:// HTML preview wires this up — it triggers the
