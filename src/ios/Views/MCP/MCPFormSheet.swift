    ...
    private var stdioSection: some View {
        Group {
            ...
            Section {
                ...
                Text("How long to wait for this server's first initialize. Raise it for slow-starting servers (e.g. uvx). Leave blank for the default (60s). Moonveil setting, not part of the MCP protocol.")
            }
        }
    }
    ...
    private func populate() {
        ...
        if server.isSTDIO {
            ...
            if let oauth = server.oauth {
                ...
                // reading the Keychain, so `moonveil-mcp-cli add
