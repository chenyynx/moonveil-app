...
object AgentTools {
    ...
    private fun browserUseDefinition(): AgentToolDefinition = AgentToolDefinition(
        ...
        description = "Control a web browser with up to 3 tabs. " +
            "Do NOT use this tool for moonveil:// action URLs (open_terminal, views, settings) — those are app deep links, use Markdown links in chat instead. " +
            "The browser supports both web URLs and moonveil:// resource URLs. Use moonveil:// URLs to preview session files (e.g. navigate to moonveil://workspace/index.html). " +
            "Sub-resources (JS, CSS, images, fonts) referenced via moonveil:// absolute paths or relative paths within HTML pages resolve correctly. " +
            ...
            "fetch to download files/resources using the page's session (returns metadata and a moonveil:// URL), " +
