...
/**
 * Coil Fetcher that resolves `moonveil://` URIs to local files.
 *
 * Usage: Register with ImageLoader.Builder().components {
 *     add(MinisImageFetcher.Factory())
 * }
 *
 * moonveil://attachments/foo.jpg → /var/minis/attachments/foo.jpg → host path
 */
class MinisImageFetcher(
    ...
    override suspend fun fetch(): FetchResult {
        // Strip moonveil:// prefix, drop any ?query, and percent-decode so
        // Chinese/emoji/space filenames resolve to the actual on-disk file.
        val stripped = uri.removePrefix("moonveil://").substringBefore('?')
    ...
    class Factory : Fetcher.Factory<String> {
        override fun create(data: String, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (!data.startsWith("moonveil://")) return null
     ...
     * StringMapper converts an `AsyncImage(model = "moonveil://…")` String into
    ...
    class UriFactory : Fetcher.Factory<Uri> {
        override fun create(data: Uri, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (data.scheme != "moonveil") return null
     ...
     * `moonveil://` file is overwritten in-place (e.g. Grok regenerating an
