...
/**
 * Parses the OSC 1337 `MoonveilOpenURL` escape sequence emitted by the rootfs
 * shim at `/usr/local/bin/moonveil-open`. The shim stands in for `xdg-open`,
 ...
 * http(s)/about, file preview for moonveil://).
 *
 * Mirrors iOS `MinisURLMarker` in `AIChatViewModel.swift`.
 */
object MinisUrlMarker {
    /**
     * ESC ] 1337 ; MoonveilOpenURL = <url> (BEL | ESC \)
     *
     ...
     */
    private val PATTERN = Regex("\u001B]1337;MoonveilOpenURL=([^\u0007\u001B]*)(?:\u0007|\u001B\\\\)")
    ...
    fun extract(text: String): Pair<String, List<String>> {
        if (!text.contains("MoonveilOpenURL=")) return text to emptyList()
    ...
    fun stripMarkers(text: String): String {
        if (!text.contains("MoonveilOpenURL=")) return text
    ...
    fun extractUrl(text: String): String? {
        if (!text.contains("MoonveilOpenURL=")) return null
