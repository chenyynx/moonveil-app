...
`moonveil://` is a **session-scoped** unified resource locator for persistent, addressable resources within the Minis ecosystem. It bridges three layers — the AI agent, the iSH Linux shell, and the iOS host app — with a single URL that works in tool results, Markdown rendering, and inter-component references.

**`moonveil://` URLs are inherently session-bound.** Every resolution, read, write, and render operation is performed in the context of the current active session. There is no cross-session resource visibility — a `moonveil://attachments/photo.png` in Session A and the same URL in Session B refer to completely independent files. The agent, the shell, and the UI all see only the resources belonging to the active session.

### Design Principles

1. **Session-scoped**: All resource access is bound to the current session. `moonveil://` URLs are opaque identifiers — they carry no session ID, because they are always resolved against the active session's storage. Cross-session access is neither supported nor exposed.
2. **Agent-first**: Every persistent resource produced by a tool MUST be returned to the model as a `moonveil://` URL so the agent can reference it in subsequent turns.
3. **Render-ready**: The chat UI resolves `moonveil://` URLs inline — images, audio, video, and downloadable files render natively in Markdown.
...
```
moonveil://<namespace>/<path>
```

| Component     | Description |
|---------------|-------------|
| `moonveil://`    | Scheme. Always lowercase. |
...
```
moonveil://attachments/screenshot.png
moonveil://attachments/photos/vacation/img_001.jpg
moonveil://workspace/report.csv
moonveil://workspace/project/src/main.py
moonveil://offloads/shell_execute_1707000000_abc12345.txt
moonveil://browser/snapshot_1707000000.jpg
```

## 3. Session Model

`moonveil://` is a **session-relative** addressing scheme. The same URL string in different sessions resolves to different physical files.
...
Every `moonveil://` URL is resolved with an implicit session context:
...
- **No cross-session reads:** An agent cannot access files from another session. There is no `moonveil://other-session/...` syntax.
- **No cross-session writes:** Writing to `/var/minis/` always targets the active session's storage.
- **Session deletion:** When a session is deleted, its entire `Library/MinisChat/minis/<sessionId>/` tree is removed. All `moonveil://` URLs from that session become permanently unresolvable.
- **History rendering:** When viewing chat history from a past session, `moonveil://` URLs resolve against that session's persisted files (loaded on session switch).
...
**Markdown usage:**
```markdown
![description](moonveil://attachments/filename.png)
![audio](moonveil://attachments/recording.mp3)
![video](moonveil://attachments/demo.mp4)
```
...
**Markdown usage:**
```markdown
[report.csv](moonveil://workspace/report.csv)
```
...
### 5.1 `moonveil://` → Linux Path

```
moonveil://<namespace>/<path>  →  /var/minis/<namespace>/<path>
```
...
**Core rule:** When a tool action produces or modifies a persistent, addressable resource, the tool result MUST include the `moonveil://` URL so the model can reference it.
...
```
Wrote to /var/minis/attachments/chart.png (1234 bytes)
minis_url: moonveil://attachments/chart.png
```
...
if path.hasPrefix("/var/minis/") {
    ...
    result += "\nminis_url: moonveil://\(namespace)\(rest)"
}
```

### 6.2 `shell_execute` — Post-Scan Enhancement

After a `shell_execute` completes, scan for new or modified files under `/var/minis/` and append their `moonveil://` URLs to the tool result.
...
[minis] New files:
  moonveil://attachments/output.png
  moonveil://workspace/results.json
```
...
minis_url: moonveil://browser/screenshot_1707000000.jpg
```
...
**Enhancement:** Add `moonveil://` URL for consistency:
```
minis_url: moonveil://offloads/shell_execute_1707000000_abc12345.txt
```

### 6.5 User Attachment URLs

When the user attaches an image/file via the input bar and it's saved to `/var/minis/attachments/`, the `moonveil://` URL is included in the user message context so the agent knows the file path.

Already implemented via `saveAttachment()` → returns `moonveil://attachments/<filename>`.
...
The `MinisImageProvider` in `AIChatView.swift` handles all `moonveil://` URLs in Markdown image syntax (`![](moonveil://...)`). It dispatches based on file extension:
...
Markdown links with `moonveil://` scheme (`[name](moonveil://...)`) should be rendered as tappable file chips that:
...
```swift
// moonveil://attachments/photos/img.jpg → /var/minis/attachments/photos/img.jpg
...
- [ ] Save browser screenshots to `/var/minis/browser/` with `moonveil://` URL in tool result
- [ ] Add `browser` directory to `mountMinisSubdir` calls

### Phase 3: Enhanced Rendering
- [ ] Implement tappable file chip for `moonveil://` links (non-image Markdown links)
...
- [ ] Consider `moonveil://` URL auto-complete or suggestion in agent context
...
- **No session ID in URLs**: Session IDs are never exposed in `moonveil://` URLs, tool results, or system prompts. This prevents the agent from attempting cross-session references.
