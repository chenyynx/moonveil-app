...
- **诚实占位 / 未完成（不假造）**：
  - `editCreation`（官方「编辑已发回合 → 回填新会话草稿」）：官方依赖 `NewSessionModel.restoreCreationDraft`，本仓新会话页是 facade 版 `RemoteNewSessionModel`，无草稿回填入口 → Glue 不实现、`SessionChatModel.onEditCreation` 保持 nil（官方件对 nil 短路）。只跳页面会静默丢待发内容，故宁可不接。
  ...
  - timeline 行内 `file:行号` 引用 chip 的点击跳转（官方跳 Workspace 预览）：本批只搬 chip 外观，跳链未接（P3）。
