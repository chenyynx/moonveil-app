...
- 🔴 **台账纠错（Qoder BATCH-C 复测，2026-09-22）**：上句「ThinkingIcon 代码引用全域清零」**判案当时成立、随后同日即被推翻**，现在是过期断言——TRR 批落点 `6428753`（09-17 13:58）处 `git grep -c 'Image("ThinkingIcon")' 6428753 -- src/ios` = **0**；同日 23:22 `011a442`「thinking-slot semantics v3 + **icon skin pairing**」按"classic=SF 原版"设计**重新引入**该引用（`ToolLiveSheet.swift:683` 注释自证「Classic=SF 原版（含 ThinkingIcon）」）。现值 **7 处活调用**：ModelGroupDetailView:259 / ToolLiveSheet:709 / AssistantBlockView:884,1224 / AIChatView:2532,4558,6231（核实命令 `grep -rn 'Image("ThinkingIcon")' src/ios`）。后人引用"全域清零"作删除/改名依据前必须重测；这是 **icon 皮肤配对的有意状态**，不是漏网。
...
- 🔴 **台账纠错（Qoder BATCH-A/C 收口，2026-09-22）**：上条「待 pp/装机」清单末项 **StatRow ThinkingIcon 是陈旧待办残留**，与同节上文「StatRow 处 ThinkingIcon 已换（E2 补批，pp 令 2026-09-17）」段自相矛盾——该件已随 E2 补批做完（落点 `6428753`），待办项未清。现状实测（工作区）：`AIChatView.swift:6225` StatRow Thinking 行 `customIcon: AnyView(` 内为 [ICON-SKIN-A] 皮肤分支（`ToolRenderStyleStore.current == .new` → `AppSymbol("sparkles", size: 14)`，否则 → `Image("ThinkingIcon").resizable()`，`011a442` 引入，见本节 ThinkingIcon 纠错条），E2 补批当时的无条件 `AnyView(AppSymbol("sparkles"))` 已被其覆盖；`grep -rn 'StatRow' src/ios` 除 :6225 外其余行均不传 customIcon。ThinkingIcon 现值 **7 处活调用**（与上方 :535 纠错条清单逐行一致）：`ModelGroupDetailView:259` / `ToolLiveSheet:709` / `AssistantBlockView:884,1224` / `AIChatView:2532,4558,6231`（核实命令 `grep -rn 'Image("ThinkingIcon")' src/ios`）。属「判案已完成、待办未销」，本条即销项，无需再装机处置。
...
- 🔴 **台账纠错（Qoder BATCH-C 复测，2026-09-22）**：上条证据 ①「全仓 grep GlassEffectContainer/glassEffectID 零命中」**写下来当天就是假的**（非"后来环境变了"）——本批落点 `11b7561`（09-19）处 `git grep -c 'GlassEffectContainer' 11b7561 -- src/ios` 命中 `ContentView.swift`(3) / `ModeTabPicker.swift`(2) / `RootModeTabsView.swift`(2)，`glassEffectID` 同形命中 8 处；这些用法是 09-16/17 的 B16-GLASS-CONTAINER 批引入的（本文件 :311 起有整节登记，与本条自相矛盾）。**结论本身仍成立但证据范围写错**：假说问的是"工具条+输入框这两块玻璃是否被同一容器合并"，正确判据 = 定点 grep——`ToolLiveSheet.swift`/`AIChatView.swift` 两文件在 `11b7561` 与 HEAD 均为 **0 命中**（`grep -c GlassEffectContainer src/ios/Views/Chat/{ToolLiveSheet,AIChatView}.swift` → 0/0）。留此字据：证伪"合并假说"要用定点证据，全仓断言必须先真跑全仓 grep。
...
- 🔴 **台账纠错（Qoder BATCH-C 复测，2026-09-22）**：「全项目 7 处 `SelectableMarkdownView(` 调用」**判案当时成立、已过期**——本批落点 `9437748`（09-20 00:39）处 `git grep` 恰为 7 处；当日 P1-CHAT 批（`6582129`）搬运远端 timeline 行新增 2 处调用（归因经 BATCH-A/C 收口复测修正：初稿作 `1f76b55c`/`6582129` 两源并列——实测 `git grep -c 'SelectableMarkdownView(' 1f76b55 -- src/ios` 合计 **7**（RemoteSessions 目录零命中），`6582129` 处合计 **9**，新增两处 SessionTimelineRow/SessionTimelineEventView 全部由 `6582129` 引入，`1f76b55` 多余归因删除；其 8 位写法亦不合全台账 7 位短 sha 口径），现值 **9**（`grep -rn 'SelectableMarkdownView(' src/ios | grep -v 'struct\|func \|init('`）：新增点 = `Views/RemoteSessions/SessionTimelineRow.swift:71`、`SessionTimelineEventView.swift`（行号以判定时为准，该文件正被并行批次触碰）——两者均走 `SessionFileReferenceLinks.rewrite` 后传入，未传 `codeBlockAutoHeight` → 默认 false，与本批"聊天流默认位不变"的申报语义一致，无回归。
...
- 🔴 **台账纠错（Qoder BATCH-C 复测，2026-09-22）**：上句审查依据「deployment target **26.2** 下 API 可用性」**写下来当天就是错的**——实测 `project.pbxproj`（按 XCConfigurationList→XCBuildConfiguration 逐块解析）：app target `Minis`（E51000072/73）= **17.0/17.0**（DEP-17 于 09-15 已把 16.0→17.0，本批 09-20 在其后），`MinisShare`/`MinisFileProvider` = 16.0、`AgentWidgetExtension` = 16.2、project 级 = 16.0；**26.2 只存在于 `MinisUITests`/`MinisTests` 两个测试 target**（26.2 字面值行 = pbxproj `:4266/:4286/:4318/:4347`，`grep -n 'IPHONEOS_DEPLOYMENT_TARGET = 26.2' src/ios/Minis.xcodeproj/project.pbxproj` 实测恰此 4 行；初稿写的 :4254/:4278/:4298/:4331 是四个 XCBuildConfiguration **块起始行**，行号歧义已由 BATCH-A/C 收口复测改记值所在行，偏移分别为 +12/+8/+20/+16）。"对着 26.2 核 API 可用性"对上架产物（app target 17.0）无效，该批 iOS18+/26 API 的可用性结论不可信按 26.2 采信——本抽屉后续件（RemoteNewSessionReveal 等）已按 17.0 立 `#available(iOS 18.0, *)` 守卫（见 NEWSESSION-UI 节），方向相反但结果侥幸覆盖；后人审远端线 API 守卫一律以 app target 17.0 为准。核实命令：`grep -n 'IPHONEOS_DEPLOYMENT_TARGET' src/ios/Minis.xcodeproj/project.pbxproj` + 逐块归属（本文实测输出见 BATCH-C 报告）。
...
## P1-CHAT — 远端会话聊天页全链路接入（AA Views/Chat 全家 + timeline 数据层 + Workspace 文件页）（Qoder, 2026-09-20，pp：「远端线全链路审查 + AA 官方页面/功能全量接入」）

- **背景**：此前远端线只到「列表页 + 详情页 sheet + 新会话页」，点开会话进的是我们的 `RemoteSessionDetailSheet`（简化版），官方聊天页（timeline / composer / 交互卡 / 通知 / 工作区文件）整体缺位。本批把官方 Chat 页与其数据/事件依赖闭包接通，并按 §0f 用本仓 Markdown 组件替换官方渲染层。**全程未 commit（提交门禁），产物留盘等 Doris 审查。**
- **官方对照（origin-cache ios/…，`v2.0.0-27-g1bc11f45`）**：
  - 页面：`Views/Chat/{SessionChatView, ChatTimelineView, SessionTimelineRow, SessionTimelineEventView, SessionInteractionCard, SessionInteractionContent, SessionInteractionDetailsSheet, SessionInteractionDock, SessionNoticesSheet, SessionTurnActions, SessionTurnReviewSheet, SessionDetailsSheet, ChatErrorToasts, ChatMessageAttachments, ChatPageContentColumn, ChatPageScrollEdge, Composer/SessionTakeoverConfirmation}.swift`；Workspace 家族：`Views/Devices/{WorkspaceFilesSheet, WorkspaceFilePreviewSheet, WorkspaceFileExportPicker, WorkspacePreviewDownloads}.swift`；组件：`Views/Components/CompletionFeedback.swift`。
  - 数据/事件层（AAV2 冻结区，逐字节）：`Business/`×9（Account/Attachment/Dashboard/DeviceManagement/DevicePairing/RuntimeInteraction/SessionDetail/SessionExport/WorkspaceFiles）、`Repositories/`×5（ClientScope/DashboardRepository/LocalStore·LocalArchive(既有)+RestorationStore/SessionProjection/SessionRepository）、`Models/Chat/`×25、`Models/Session/`×2、`Models/Devices/WorkspaceDirectoryModel.swift`、`Domain/Connector/V2DeviceManagement.swift`、`Network/V2ConnectivityMonitor.swift`。
  - 组合根：`Business/V2ClientServices.swift` → Glue 新文件 `V2RemoteChatServices.swift`（成员剪枝台账写在文件头，逐条给「本仓该职责另有归属」的字据）。
- **落地**：
  - **25 新 app 文件**（平铺 `src/ios/Views/RemoteSessions/`，子目录会触发 audit-missing）= 22 个官方逐字搬运件 + 3 个自造适配件（`RemoteGlassIfAvailable` / `RemoteSelectableText` / `SidebarDrawerEnvironmentKeys`，见「合法差异」）；既有文件改动 3 个（`ChatComposerDock` +13 行、`ComposerOptionsSheet` +35 行、`RemoteSessionListView` 入口接线）+ `RemoteKit/Package.swift`。每文件与官方逐字对照的差异行数见自审报告附表（0–175 行，全部为守卫/渲染桥接/文案落地，无逻辑裁剪）。
  - **入口接线**（`RemoteSessionListView`）：`showsChat` + `chatSessionId` → `.navigationDestination(isPresented:)`（与 `showsDeviceDetail` 同款），打开会话时 `markRead` → 进官方 `SessionChatView`；设备名 = `listConnectors()` 字典（官方 `appState.connectors.first { $0.id == connectorID }?.name` 等价）；旧 `.sheet(RemoteSessionDetailSheet)` 从列表入口下线（设备页入口保留，未动该文件）。
  - **离线缓存**：Glue `restoreCache(selection:)` 按官方签名逐字（dashboard + `sessionRepository.restoreCachedSession(id:)`），调用点 = chatDestination `.task(id:)`（本仓「当前选择」即打开的会话；官方在 `AppState:85` 创建组合根后立刻调，语义同）。
  - **§0d 冻结区**：AAV2 新增 41 个 `.swift`（本批），全部与 origin-cache 字节一致，`FREEZE-MANIFEST.txt` 现 **97 行 = 97 文件**，`sha256sum -c` 97/97 OK。
  - 🔴 **台账纠错（Qoder BATCH-C 复测，2026-09-22）**：本句与「验证」段的 97/97 均**判案当时成立、已过期**——P1 落点 `6582129` 处 manifest 恰 97 行（`git show 6582129:Packages/RemoteKit/Sources/AAV2/FREEZE-MANIFEST.txt | wc -l` = 97）；其后 `6de8bbf` 补搬 ReplyPresentation（+1，另夹带 5 行未提交 P2 登记、被 `bfa13e8` 摘除至 98）与 P2 批 `7baccae` +5，现值 **103 行 = 103 文件**。实测（09-22，于 `Packages/RemoteKit/Sources/AAV2/`）：`sha256sum -c FREEZE-MANIFEST.txt` → **103 行 OK、非 OK 行 0**（`sha256sum -c FREEZE-MANIFEST.txt 2>/dev/null | grep -c ': OK$'` = 103）。勿再以 97 为准。
  - **四处接线**：`pbxproj`（PBXBuildFile 带 `COMPILER_FLAGS = "-default-isolation MainActor"` / PBXFileReference / group children / app target Sources phase）+ `Package.swift` `sources:` 白名单（新增 `AAV2/Repositories`、`AAV2/Models/Session`、`AAV2/Models/Devices`；`AAV2/Models/Chat` 批9 已登记）。本批补 **9 个 Xcode 侧漏挂文件**：磁盘已有但未挂 pbxproj 的 5 个（`Repositories/{V2SessionRepository,V2LocalStore,V2LocalArchive}`、`Models/Session/{V2SessionModel,V2SessionReadCoordinator}`——这 5 个上游批已 commit 但两层都未接，属「ported, not wired」台账态，本批正式接入）+ 依赖闭包审计新搬的 4 个（`Repositories/V2SessionProjection`、`Models/Chat/{ChatShellSelection,NewSessionSubmission,ProjectSidebarPreferences}`），并新建 `Models/Session` PBXGroup——即 e850456 那类「Xcode cannot find type」的根因（Package.swift 用目录白名单会整目录编到，pbxproj 用逐文件列举，只补 SwiftPM 不补 Xcode 侧必挂 iOS Build）。
  - **依赖闭包审计**（脚本化）：以官方全量声明集为字典，扫本仓已接线文件里的类型引用 → 命中 `ChatShellSelection` / `NewSessionSubmission` / `ProjectSidebarPreferences` / `V2SessionProjection` / `V2SessionModel` 五个未搬声明（`V2RestorationStore` / `V2DashboardRepository` / `V2SessionRepository` 引用），补齐后复扫 = **0** 未解析官方类型引用（`Layout` 为 SwiftUI 协议，误报）。
  - **文案落地**：39 处 `String(localized:)` 由「官方源键」改为「官方 `Localizable.xcstrings` 的 zh-Hans 显示值」，与既有 COMPOSER-FULL / NEWSESSION-COPY 约定一致；复扫 remaining unresolved = 0；未新增 xcstrings 键（本仓不注册远端线新键）。
- **合法差异字据**：
  - 类①（版本守卫 + 回退）：iOS 18 `ScrollPosition` / `onScrollVisibilityChange` / `onGeometryChange` / `scrollEdgeEffectStyle`；iOS 26 `glassEffect` / `GlassEffectContainer` → `aaScrollEdgeSoft()` / `remoteGlassCard()` / `remoteGlassCapsule()`（先例 AuthGlassCompat / AppGlassButton）；`@Entry` → 传统 `EnvironmentKey`（`SidebarDrawerEnvironmentKeys`：只声明官方两个详情页消费键，本仓无抽屉壳 → 恒 false，官方分支逐字保留并自然短路）。
  - 类②（官方依赖本仓不具备的能力）：**§0f 渲染拍板**——官方 Markdown 家族 + Textual 不得落地，timeline 行内富文本改接本仓 `SelectableMarkdownView` 族；官方 `ChatSelectableText` 依赖 `InlineText` + `.textual.textSelection` → 本仓 `RemoteSelectableText.swift` 用 `UITextView(isSelectable)` 还原同一语义（局部范围选择、逐字不解释标记），符号沿用官方名，三处调用点给与官方等值样式；官方 iOS 26 `Layout` 协议件（`ComposerLayout`）在 17.0 走 GeometryReader 等价路径。
  - 守卫删除：`SessionTimelineRow` 去 `.environment(\.chatLayoutTraceOwner, row.id)`（官方布局探针基建，本仓无消费方）。
  - 冻结基线偏差（需 Doris 处置）：强模式 `MV_CLOUD=…/moonveil-cloud scripts/aav2-freeze-check.sh` 报 `Models/Chat/MarkdownBlockSizing.swift` / `Models/Chat/TimelineHistoryPull.swift` 两文件 ≠ tag `v2.0.0`——二者与官方源 origin-cache **字节一致**，差异来自 origin-cache HEAD 已前进到 `v2.0.0-27`（含 Doris 自己的 `1bc11f45 [doris] Add explicit init(edge:)…` 与上游 `ba50eb35 fix(ios): skip redundant static text layout…`）。CI 走 manifest 模式为绿；建议要么把 `AAV2_BASE_TAG` 抬到 ios-ci 对应 commit，要么在 gate 里对这两文件加白名单说明。
- **诚实占位 / 未完成（不假造）**：
  ...
  - 🔴 **台账纠错（Qoder BATCH-C 复测，2026-09-22）**：本条已作废且与上文「后台落盘钩子」段（「`sessionReads` 从组合根剪枝名单移出并全接」）**自相矛盾**——实测该段是对的：`V2RemoteChatServices.swift` 构造 :82 + reconcile 钩子 :98（`dashboardRepository.reconcile → sessionReads.ingest`）+ updateConnectivity :108 + `setAppInBackground` 内 `sessionReads.setActive(!background)` :237；`RemoteSessionListView.swift` setVisibleSession :228 + onChange :233（`grep -n sessionReads Packages/RemoteKit/Sources/Glue/V2RemoteChatServices.swift src/ios/Views/RemoteSessions/RemoteSessionListView.swift`）。本条是 P1 初稿残留，Doris 09-21 接线补丁（上文「后台落盘钩子」已装配段）写入了新事实却没删旧占位行。后人以代码与本节「已装配」段为准，勿被此条误导去"补做 P3"。
  - P2 未做：`RuntimeConfigurationSheet` / `ConnectorCredentialSheet` / `AddDeviceAgentSheet` / 设备详情 Agent 区（现仍为真实跳转的过渡实现）。**本批已查明硬前置**：官方三个设备写操作（重命名 / 轮换凭证 / 删除）都以 `connector: V2Connector` 为单位（`DeviceManagementView.swift:142/262`，门 = `canManage = dashboard.canWrite && …`），而本仓 `RemoteDeviceDetailView(service:)` 无设备身份、数据取共享 loader（跨设备混排）→ 必须先做「设备页改 per-connector」，否则破坏性操作会打到错设备；该改动同时是 pp 已验收页面的结构改动，需先过 pp。依赖闭包已核实可满足（`V2ConnectorRevokeResponse` / `V2DeviceManagementService` / `V2DeviceProjection` 均在本仓，另需逐字搬 `Models/Chat/{DeviceManagementModel,SessionSidebarPresentation}.swift`）。P3 未做：列表多选批量归档、`nextCursor` 分页（首版 limit=100）。
- **死隔离**：改动面 = `src/ios/Views/RemoteSessions/`（25 新 + 3 改，共 28 文件）+ `Packages/RemoteKit/`（Glue 1 新 + 2 改、AAV2 新增 41 全为逐字搬运 + manifest 登记、Package.swift 白名单）+ `src/ios/Minis.xcodeproj/project.pbxproj`；`ContentView` / `ChatStore` / `AIChat*` / 本机线零改动（`git status` 可核）。
- **回归项**：① 列表点会话 → 官方聊天页（timeline 首屏 100 条 + 上滑分页 ② 实时帧增量渲染 ③ 交互卡批准/拒绝/自定义输入 → `runtime.notice/action` 回包 ④ 通知 sheet / 回合审阅 sheet / 会话详情 sheet ⑤ 接管确认 + takeover 后才可发送 ⑥ composer 附件上传 + 模型/权限选择 ⑦ 错误 toast（离线 / 发送失败 / 恢复）⑧ 工作区文件页：目录浏览 → 预览（图片走 WKWebView / 文本）→ 导出分享 ⑨ Markdown 渲染 = 本仓组件（禁止 Textual 残留符号）⑩ iOS 17 真机无玻璃/无 scrollEdge 的降级外观 ⑪ 深色模式 ⑫ 离线冷启动：缓存先铺（restoreCache）再刷新覆盖。
- **验证**：静态全绿——`scripts/import-scan.sh` OK；`scripts/aav2-freeze-check.sh`（CI 模式）OK + `sha256sum -c` 97/97；`scripts/fork-point-audit.sh` OK；`scripts/audit-project-inputs.py` `checked=608 missing=0 generated-ok=2`；ruby `xcodeproj` 复核 = RemoteKit 磁盘 102 文件全部在 app target Sources phase、RemoteSessions 目录无 NOT-IN-PHASE、除 2 个构建期生成文件外无悬空路径；本批 73 个新增/改动 `.swift` `swiftc -parse` **73/73 通过**。`swift build`（Linux）EXIT=1，两类错误均环境性/既有：① `no such module 'Network'`（Apple 专有 framework）② `APIClient.swift` 逗号解析告警——`git show HEAD:…APIClient.swift | swiftc -parse -` 同样复现，本地 swift-6.0.3 工具链怪癖，CI Xcode 26.2 接受。**权威编译 = CI remotekit-build + ios-build，装机回归待做。**

## PATCHES-P2 — 设备管理全链路（per-connector + 官方写操作 + Runtime 配置全家）（Qoder, 2026-09-21，任务书「P2 设备管理全链路 + P3 列表收尾」；全程未 commit，等 Doris 复审）

- **背景**：P1 台账留了两条未做——① `RuntimeConfigurationSheet` / `ConnectorCredentialSheet` / `AddDeviceAgentSheet` 未搬，设备页 Agent 区是"真实跳转"过渡；② 设备页无设备身份（`RemoteDeviceDetailView(service:)` 读共享 loader 跨设备混排），破坏性写操作无从接。本批两件一起做：**P2-A per-connector 是硬前置**。
- **P2-A（设备页改 per-connector）**：
  - `RemoteDeviceDetailView` 新签名 `init(service:connector:onDeleted:onSessionsUpdated:)`——对齐官方 `DeviceManagementView { let connector: V2Connector; ... }`。所有数据面改从 `connector` 派生：`model.updateSessions(connectorId:allSessions:)`（官方 `V2DeviceProjection.sessions` 过滤 + `SessionSidebarPresentation.precedes` 排序，逐字）、`deviceProjects`（官方排序键三组逐字）、agent 区 = `services.agents(on: connector.id)`。
  - 调用点（`RemoteSessionListView`）：`deviceConnector = connectors.first(where: online) ?? first`（本仓预览期单终端卡定稿），`navigationDestination` 组内容 `if let connector = deviceConnector, let services = service.chat { RemoteDeviceDetailView(...).id(connector.id) }`；删除回调 `services.removeConnector(connectorId:)` + pop。
  - pp 已验收视觉骨架保持：`RemotePalette` / `groupedCard` / `unavailableCard` / `contentSwitcher` 样式 / `fullScreenCover` 新会话 / `RootTabRouter` 页切栅栏全部未动。**视觉级变动仅一处**：Agent 区容器由自造行改为官方 `DeviceOverviewSections { DeviceAgentSection(...) }`（GroupBox 圆角卡 + 官方行结构）——自审报告单列段。
- **P2-B（官方组件搬运 + 三写操作）**：
  - **AAV2 冻结区 +5**（`Models/Chat/`，与 origin-cache 逐字节一致，`sha256sum -c` 103/103 OK）：`DeviceManagementModel` / `RuntimeConfigurationModel` / `DeviceAgentModel` / `SessionSidebarPresentation` / `RuntimeConfigurationRows`。
  - **app 层平铺新 8 件**（`src/ios/Views/RemoteSessions/`，官方 `Views/Devices/` 逐字搬运，头注 = 本仓惯例）：`ConnectorCredentialSheet` / `RuntimeConfigurationSheet` / `RuntimeConfigurationFieldView` / `RuntimeConfigurationRowEditors` / `DeviceAgentSection`（含 `AgentRediscoveryButton` + `AgentSetupSheet`）/ `AddDeviceAgentSheet` / `DeviceOverviewSections` / `DeviceSessionSelectionDock`（官方结构体住 `DeviceOverviewContent.swift`，按模块化规则单拎成件）。diff 行数与差异字据见自审报告附表（0–56 行/件，全部为版本守卫/文案落地/文件拆分，无逻辑裁剪）。
  - **三写操作（以 connector 为单位，官方语义逐字）**：重命名 `model.renameConnector` → `onConnectorUpdated`；轮换 `model.revokeConnector` → 成功后 `credential` sheet 展示新凭据（`ConnectorCredentialSheet`，serverURL 取 `service.serverURLValue`）；删除 `model.deleteConnector` → `onDeleted(connector.id)` 组合根移除 + pop。门 = 官方 `canManage = dashboard.canWrite && !isDeviceActionRunning && !isArchiveActionRunning && !busy`；`canReadFiles = canWrite && connector.status == .online`；**无权限是禁用态不是隐藏**（官方 `.disabled(!canManage)` 逐字）。
  - **PAIRING-FULL 过渡替换**：列表页旧 `onChange(agentSetup.presentedConnector?.id) { showsDeviceDetail = true }` 跳转删除，改官方 `ChatShellView:53-59` 形状——`.sheet(item: agentSetupBinding)` → `AgentSetupSheet(connector: <facade id 映射 V2Connector>, model: services.agents(on:), onFinish:)`；`RuntimeConfigurationSheet` / `AddDeviceAgentSheet` 由 `DeviceAgentSection` 官方链路驱动。
  - **官方四 alert 全接**：重命名设备 / 更换 Connector 凭据？（轮换确认）/ 删除设备？/ 归档会话？+ 项目归档·删除 alert + 会话操作失败 alert（`sessionActionError`，见下）。
- **Glue（`V2RemoteChatServices`）扩员**（官方 `AppState` / `ChatShellDelegate` 半段宿主）：`agents(on:)` / `updateAgentConnections()`（官方逐字，去 `agentSetup` 半段——本仓配对另有宿主 `AgentSetupCoordinator`，字据在文件头）/ `updateConnector(_:)` / `removeConnector(connectorId:)` / `updateSessions(_:)` / `setSessionsArchived(sessionIds:archived:)`（官方 `AppState:396-418`，去 `cachedServices===` 换代守卫——单一组合根本仓无换代语义，注释字据）/ `sessionActionError` + `dismissSessionActionError()`；init 的 `connectivity.onChange` 内补 `updateAgentConnections()`（官方 `V2ClientServices:97` 同位）。`sessionActionError` 反应性：组合根非 Observable → 页面 `performArchive` 完成后取回落本地 `@State` 镜像，alert 形状保持官方（字据在设备页文件头）。
- **文案落地**：全部 `String(localized:)` → 官方 `Localizable.xcstrings` zh-Hans 显示值（对照表见自审报告 B6）；`RuntimeConfigCopy` 运行时动态查表键 20 个注册进 `src/ios/Localizable.xcstrings`（纯插入 340 行，总数 2320；`extractionState: manual` + en=键自身）。**官方未注册 zh 的键保持源串显示，不自创翻译**：`'%lld selected'`、`"Copy %@"`、`"Delete connector?"`（字据见报告）。
- **pbxproj**：13 新文件 × 四处挂载（app 8 件 `A220-A227/B220-B227` 带 `-default-isolation MainActor`；AAV2 5 件 `RK+24hex`，BuildFile 无 settings——`ChatToastStore` 先例）。三重核验脚本全绿（四处各恰 1 次、无孤儿 fileRef、组归属正确）。
- **诚实 Staged（不假造型）**：工作目录模式列表维持空态（`WorkspaceDirectoryChoice` / `WorkspaceFilesSheet` 目录选择链未在本批搬运范围）；「重命名项目」contextMenu 维持禁用（官方 `ProjectEditorSheet` 未搬）；agent inventory 的 public 刷新面走 `DeviceAgentModel.refresh`（官方同款），无数据时零行空态。
- **死隔离**：改动面 = `src/ios/Views/RemoteSessions/`（8 新 + 3 改）+ `Packages/RemoteKit/`（Glue 1 改 + AAV2 5 新逐字）+ `src/ios/Localizable.xcstrings`（纯插入）+ `pbxproj`；本机线（`ContentView` / `Views/Chat` / `AIChat*` / `MessageList`）零改动。
- **验证**：17 个改动/新增 `.swift` `swiftc -parse` 全过；`sha256sum -c FREEZE-MANIFEST.txt` 103/103 OK；`audit-project-inputs.py` `checked=623 missing=0`；越界 grep 零命中。权威编译 = CI ios-build；装机回归待做（清单见自审报告）。

## PATCHES-P3 — 远端列表收尾：nextCursor 分页 + 批量归档 dock + 错误 toast 细化（Qoder, 2026-09-21，同批任务书）

- **P3-1 列表分页（nextCursor）**：`RemoteSessionLoader` 游标状态（`activeCursor/activeHasMore/archivedCursor/archivedHasMore` + `hasMorePages` 按 `lastFilter` 分支）；`load()` 三态改为捕获完整 page（`.all` 用 `async let` 双路各取一页）；新 `loadMore(service:)`（guard `.loaded && !isLoadingMore`，active 优先再 archived；失败只设 `loadFailure` 不改 phase）+ `appendSessions`（按 id 去重 + 复用 `resort`）。列表页卡堆尾 `if loader.hasMorePages { ProgressView 行 }.onAppear { loadMore }` 滚到底拉下一页。**差异字据**：官方 iOS 列表无分页消费方（侧栏一次全量），本实现按服务端 `nextCursor` 分页契约落地。
- **P3-2 多选批量归档 dock**：官方位置查证——`DeviceSessionSelectionDock` 只挂设备页 sessions tab（`DeviceManagementView.safeAreaInset`），故落 `RemoteDeviceDetailView`：`⋯` 菜单「选择会话/完成」+ 200 上限提示（官方 `Select no more than 200 sessions at once.` zh 已注册），选择态行前 checkmark、dock 计数 + 归档/取消归档按钮 → `performArchive` → 组合根 `setSessionsArchived`；`archiveAll` 官方双分支逐字（project 范围走 `dashboard.archiveProject`，connector 范围走 `model.archiveSessions`）。
- **P3-3 错误 toast 细化**：`RemoteSessionLoader` 新增 `loadFailure/writeFailure: V2ClientFailure?`（网络中断/鉴权失效/设备离线由冻结件 `V2ClientFailure(_ error:)` 分类，文案标题由冻结件 `ChatToastStore` kind→zh 映射：`invalidResponse`→会话数据格式不兼容 / `authentication`→登录状态需要验证 / `offline`→网络已断开 / default→操作未完成——**逐字官方，未自创**）。列表页 `.overlay(alignment:.top) { ChatErrorToasts(store:isRetrying:onRetry:) }`，`loadFailure` → source `sync`（带重试钮 = `loader.refresh` 显式重读，官方非重放语义）、`writeFailure` → source `operation`；设备页四源（agents/device/sync/action）照官方。既有 `writeError`/session 类展示保持兼容（`markWriteFailure` 统一三处写回滚双写新旧通道）。
- **回归项（P2+P3 合并）**：① 长按/点终端卡 → 该设备详情页（数据仅该 connector 会话/项目）② 重命名后顶栏标题即时更新 ③ 轮换凭证 → 新凭据 sheet → 桌面端旧 token 失效 ④ 删除设备 → pop 回列表 + 卡消失 ⑤ Agent 区添加/配置 Runtime → `RuntimeConfigurationSheet` 保存/恢复默认 ⑥ 配对流程 `AgentSetupSheet` 弹窗（原跳设备页过渡已删）⑦ 列表滚到底加载下一页（>100 会话设备）⑧ 设备页 sessions tab 多选 → dock 批量归档 → 已归档 tab 恢复 ⑨ 断网刷新 → 「网络已断开」toast + 重试钮恢复；归档失败 → 操作类 toast ⑩ 深色模式 + iOS 17 降级外观（dock/卡片 `remoteGlassCard`）。

## [归还] Qoder(本会话) | 2026-09-21 09:37 | iOS Build 连红 14 次的根因收口批（pp 拍板「全力解决」→ 轨 C）

- **根因判定（三层，证据链在会话内）**：
  - A=机制层：app target 本机无编译器，唯一类型检查器是 mac CI（8–18 分钟一轮），错误按依赖分层暴露 → 每轮只看见下一层；CI blob 日志 EOF，只能读 workflow 自己截断的 25×200 字符 annotation → 盲人摸象式打地鼠。
  - B=配置层：`ab4d088` 半抄官方 AA 并发配置——只设 `SWIFT_DEFAULT_ACTOR_ISOLATION=MainActor`，漏配套 `SWIFT_APPROACHABLE_CONCURRENCY=YES`；AA 用 0 个 per-file 隔离 flag，本仓自建 190 个。结果隔离错误无界爆发，其后 5 次提交全在还债（89 处 `nonisolated` 绷带）。
  - C=交付层：当前唯一拦路错 `SessionChatView.swift:72` type-check 超时；`cc9b250` 提交信息声称已拆 body，`git show` 证明该文件零改动（修了个没修）。
- 🔴 **被推翻的原方案（留字据，防后人重走）**：我最初提的「单变量 revert `ab4d088`」是错的。取证发现 `ab4d088` 修掉了 `NewSessionSubmission.swift:20` 的硬错（`62fc0ee` 有 1 错 → 之后 5 个 run 全 0），revert 会让冻结区里那行错误复活。已改为轨 C：保留 `ab4d088`，补齐官方配置对。
- **本批实际改动（工作树，HEAD=`cc9b250`，47 文件）**：
  - 轨 C（隔离口径）：`src/ios/Minis.xcodeproj/project.pbxproj` app target Debug/Release 各 +1 行 `SWIFT_APPROACHABLE_CONCURRENCY = YES;`（4539/4596）；`git apply -R` 反做 `cc9b250`/`3acb3ef`/`4459183` 三个绷带提交 → 89 处 `nonisolated` 净清零。
  - 轨 S（body 拆分）：仅 `src/ios/Views/RemoteSessions/SessionChatView.swift`，72 行的 `var body: some View` 拆为 `body`/`presentationSurface`/`chatSurface`/`rootSurface(geometryHeight:)`/`emptyHintOverlay`/`composerDock(geometryHeight:)`/`topLeadingOverlay`/`@ToolbarContentBuilder toolbarBody`/`sheetContent(_:)`；16 个 modifier 相对顺序不变，`geometry.size.height` 只被 `maximumEditorHeight` 一处消费故下传 CGFloat；先例 `7577a7d`。
  - 轨 G（Linux 侧门禁，把 15 分钟循环压到秒级）：新增 `scripts/pbxproj-strict-check.py`（严格 OpenStep 文法机：`8314d82` 精确报 55 处缺 `;`，`20ff5a7`/`9da918b` 干净）、`scripts/audit-swift-registration.py`（盘→pbxproj 反向登记审计，含 sync-group 与死代码豁免）、`scripts/swift-parse-check.sh`（581 文件全量 `-parse`，warm 4.8s，fail=0/divergence=6 已知 trailing-comma 语法差）、`scripts/swift-spelling-check.py`（`contentmodifier` 类大小写镜像启发式，suspects=0）；`ci.yml` 串接 4 个新 gate（原 7 步保留，现 11 步）；`ios-build.yml`/`remotekit-build.yml` annotation `25→50`、`200→700` 字符 + `if: failure()` 全量日志 artifact；`gh` 侧改用 `/check-runs/<id>/annotations`（`/actions/jobs/<id>/annotations` 是 404）。
  - 死代码豁免：`src/ios/ISHCommandExecutionExample.swift` 登记为上游死代码（初始提交 `d9d4d5b` 携带，零引用、从未参与编译），只加审计豁免不加 target——登记会新编译 204 行示例代码，删除需 pp 拍板。
- **验证状态（诚实口径）**：本地 9 项门禁全绿（strict-pbxproj / reverse-registration / parse / spelling / import-scan / fork-point / aav2-freeze / aa-assets / project-inputs，均 rc=0）；`swiftc -parse` 只保证语法，**type-check 超时是否消除、Xcode 26.2 是否真吃掉新 key 均未验证**——已在 `ios-build.yml` 加 `[CONFIG-PROBE]`：失败路径 PROBE 增行两把 key，成功路径把 `-showBuildSettings` 结果写进 step summary，一绿一红都能证明配置是否生效。
...
## COLD-LAUNCH-ROOT — 冷启动一律落列表根 + 本机线显式意图补 tab 路由（2026-09-21，pp 装机反馈「退了后台再打开的是本地聊天页…这个功能是原来的功能 现在加了远端这一块 有点冲突」；拍板「冷启动一律落列表根」+「同批修」）

- **现象链（两层，第二层是本批要修的）**：① 第一层已在前一批修掉——远端 push 页（终端卡详情页）泄漏横滑手势 → `router.route(to: .local)` 把 `lastTab`（UserDefaults `app.rootSourceMode`）写成 `.local`。② 冷启动时 `RootTabRouter` 从该键恢复出 `.local`，于是 `ContentView` 启动 `.task` 里的 `[LAUNCH-SESSION-TAB-GUARD]`（2026-09-20，`else if tabRouter.mode != .local`）判定"在本机线"→ **放行** `launchScreen` 默认分支 → 自动 `openSession(...)` push 上次会话 → 用户看到的"退了后台再打开的是本地聊天页"。
- 🔴 **为什么上一批的守卫是对的但不够**：`[LAUNCH-SESSION-TAB-GUARD]` 的语义（远端在前时不许偷偷 push 本机页）没问题，问题在它把**正确性押在了一个会被手势污染的输入上**（`mode`）。凡"读一个可被用户手势改写的持久态来决定要不要替用户导航"，都是同一类结构缺陷。pp 拍板改成**从源头不再自动跳页**：新分支 `[LAUNCH-ROOT-ONLY]`（`:1760-1770`）不看 `mode`、不导航，只打一条含旧设置值的日志（`was \(launchScreen)`）便于日后排查。污染无从生效，因为它已经不吃这个输入了。守卫本身随之删除——留着等于留一条依赖污染输入的死路。
- **删除的自动跳页逻辑（`switch launchScreen`，−30 行）**：`1=Last Session` / `2=New Chat` / `0=Auto`（>15min 未活跃则新建）三档 + `3=Home`（原本就是 `break`）。聊天页此后**只能由四条显式意图进入**：通知点击、分享（`onOpenURL` / `onChange(hasPendingShare)`）、快捷指令、iPad 分栏恢复；或用户在列表里亲手点一行。
- **同批第二件事（pp「同批修」）—— `[LOCAL-INTENT-TAB-ROUTE]` 7 处**：远端 tab 引入后新增的一类缺口——`ContentView` 常驻（远端在前时 `opacity 0` 但 alive，见 TAB-SWAP-FLASH），所以本机线的 `openSession` / `commitNavigationPath` 能在**屏幕还在远端**时把本机聊天页 push 进去：用户收到"任务完成"通知点一下，人还在远端页，聊天页在背后开了。修法：每个本机线导航出口都配一句 `tabRouter.route(to: .local)`。落点：
  - `:1421` `.moveInputToSession`（放 `isWideLayout` 分叉**之前**，一次覆盖 wide/compact 两个出口）
  - `:1446` 通知热路径 `.onReceive(.openSessionFromIntent)`（放在 `markHandled()` 之后、`guard 目标已可见 else return` 之前——目标已在前说明必然在本机线，路由是 no-op，不影响正确性）
  - `:1711` 通知冷启动缓冲（`NotificationNavigationStore.takePending()`）
  - `:1742` 分享冷启动（`shareCoordinator.hasPendingShare` 分支）
  - `:2005` 分享热路径 `onChange(hasPendingShare)`——**只放在"屏幕上没有会话，新建一个"的 `else` 分支**；另一条 `foreground` 路径不导航（只盖缓冲标记），无目的地可跟，切 tab 反而成了无端跳转
  - `:3536` `handleNewChatRequest()` 函数**顶部**（快捷指令「新会话」；三个出口含 `workflowEnsuringHome` 早退的 `popToHomeForQuickAction()`——早退路径若不切 tab，会在远端看不见的本机线上弹层）
  - `:3606` `openSessionForPendingQuickAction()`（本函数不止 `handleNewChatRequest` 一条来路：`QuickActionWorkflow.$state` 的 observer 会让上一次启动残留的 `ensuringHome` 超时在本轮独立触发这里，见 `[T-share-vs-shortcut-state]`，那条来路没经过前者的路由）
  - `route(to:)` 的 `didSet` 带 `guard oldValue != mode`（`RootTabRouter.swift:16-18`），已在本机线时是纯 no-op，因此 7 处无条件写不会造成多余持久化。
- **⌘N 覆盖说明**：`MinisApp.swift:439` 的 `.commands { CommandGroup(replacing: .newItem) }` "New Chat"（⌘N）发的是 `.newChatRequested`，经 `:1302` → `handleNewChatRequest()` → `:3536` 已被本批覆盖，无需单独加。远端线不共享这个出口：远端新建走 `RemoteNewSessionView` 的 `fullScreenCover`，全仓 grep 无远端侧 `.newChatRequested` 发送方（死隔离：本机意图不落到远端页，反之亦然）。
- **配套删除（pp 2026-09-21 确认「可以」后执行）**：既然三档自动跳页没了，「Launch Session」就是死控件，整条链一并拔掉——
  - `ContentView:6983-6987`：Appearance 设置页的 `Section { Picker("Launch Session") … }` 连同 header/footer 删除，原位置留一条说明「tab 的自动记忆是另一套东西，未受影响」。
  - `ContentView:1118` / `ContentView:6911`：两处 `@AppStorage("launchScreen")` 声明删除（ContentView 一处 + `AppearanceSettingsView` 一处；这两个行号是**删除前**的位置，删除后原行只剩 `:1117` 的一条说明注释）。
  - `ConfigRegistry+Builtins.swift:1086-1093`：`appearance.launchScreen`（`AppStorageIntCodedEnumField`）注册项删除；该类型另有 4 个消费者，未成死类型。全仓无字段计数/黄金名单测试，删除不破坏注册表面。
  - 5 条日志字串里对已删概念的引用改为准确措辞（`launchScreen=\(launchScreen)` 从 `.task` 摘要日志移除；"skipping launchScreen logic" → "skipping launch navigation"；"overriding launchScreen logic" → "overriding the launch chain"；`launch-root-only` 那条不再报旧值）。
  - `AppDelegate.swift:33-38` 与 `SendPromptIntent.swift:369-377 / 402-413`：三处**现在时**叙述"会落到 Launch Session 默认页"的注释改为准确表述——`NotificationNavigationStore` 的缓冲与 delegate 注册**照旧必需**（丢的是通知点击本身，不是被默认页覆盖），但成因措辞更新；同时澄清 `handledRecently` 并非只剩诊断：它仍然挡住启动链后面的分享/快捷指令分支重放这次导航。
  - `Localizable.xcstrings` 的 `"Launch Session"` 条目**未删**：该文件是导出物且本仓已容忍 439 个未引用键，无门禁校未用键，删它只增加 4.4 万行文件的噪声。
- **保留项（非本批范围）**：`[T-notification-tap-vs-launch-session]` / `[T-share-routes-to-background-session]` 等标签名里的 "launch-session" 字样不改——它们是历史事故编号，改名会断掉跨文件回溯链。
- 🔴 **自审补记（提交后追加，纠正本条目的一处措辞）**：上文把 7 个落点称作"本机线意图"，但**本机导航栈并不只渲染本机会话**——`ContentView:2166` 有一条 `if id.hasPrefix("remote:")` 分支，push 出 `AIChatView(sessionId:remoteDeviceId:)`；它的入口是 `:3877-3884` 那条挂在**本机会话列表**里的 iCloud 远端设备分组（`NavigationLink(value: "remote:\(deviceId):\(sessionId)")`）。所以准确说法是「**本地 NavigationStack 的导航出口**」而非「本机线意图」。对本批 7 个 `route(to: .local)` 无影响，逐条核过：那条路只能由看得见的手动点击进（NavigationLink，远端在前时列表 `opacity 0` + 不吃点击），且 push 时 `syncFixedBarFlags()` 把 `localAtRoot` 置 false 正是我们要的语义（齿轮让位、页切手势退出）。**留此字据防后人误读**："route 到 .local" ≠ "目标是本机会话"。
- **同次自审查清的两件事**：① `.openSessionFromIntent` / `.newChatRequested` / `.moveInputToSession` 三个名字的**全部**生产方都在本机侧（`AIChatView:906/924/1170/2031`、`MinisApp:439` ⌘N、`AppDelegate` 通知 delegate、`DeepLinkRouter:88`、`SessionsOffloadBridge:446`、`AskMinisIntent:85` / `OpenSessionIntent:16` / `SendPromptIntent:448`、`DebugJSONRPC:2722`），无一是远端线意图 → 没有"误把用户从远端拽回本机"的落点；② 通知 payload 的 `sessionId` 全部来自本机 agent 跑批（`ShortcutRunTracker:351` / `SendPromptIntent:348` / `BackgroundKeepAliveManager:883`），故热/冷两条路 push 的都是本机会话，`route(to: .local)` 与目标一致。
- **自审顺手发现的旧缺口（未修，非本批引入）**：`DeepLinkRouter.swift:76-89` 的 `sessions/<id>` 直接把 URL path 当 sessionId 传下去，若外部传 `remote:<dev>:<sess>` 会被当成本机会话 open 一个不存在的 id（无提示）。当前无生产方会传这种值，登记备查。
- **官方等价物自查**：官方 `launchScreen` 三档跳页在本仓是既有搬运物（非本批新增）；撤掉它无官方对照，属 pp 的产品决策（覆盖官方行为，已在标题注明）。`route(to:)` 配导航出口的写法亦无官方对照——官方无远端 tab，故不存在"push 到背后那条线"这类缺陷；属本仓缺陷修复。
- **死隔离四问**：①远端线零改动（`Packages/RemoteKit/**` 本批 0 字节，AAV2 冻结区 0 字节），改动 4 个本机线文件：`ContentView.swift`(+64/−74) / `ConfigRegistry+Builtins.swift`(+2/−8) / `AppDelegate.swift`(+3/−3，仅注释) / `SendPromptIntent.swift`(+10/−9，仅注释)；②唯一共享字段是 `RootTabRouter.mode`（经 `route(to:)` 写），本机线是本字段的合法主人，且 `didSet` 的 no-op 门保证不会抢远端的显式切换；③见上；④回归项见下 ①–⑧。
- **验证边界（诚实口径）**：删除设置项后 6 门**重跑**全绿 rc=0（`swift-parse-check` 581 文件 fail=0 divergence=6(已知) / `swift-spelling-check` suspects=0 / `aav2-freeze-check` OK / `import-scan` OK / `pbxproj-strict-check` objects=1681 OK / `audit-swift-registration` registered=597 orphans=0）；4 个改动文件花括号净平衡均 0、无未闭合字符串；`grep launchScreen` 确认代码级引用 0 条（余下只在注释里作历史叙述）；`AppStorageIntCodedEnumField` 仍有 4 个消费者故未成死类型。**但 Linux 侧无编译器**：type-check 与真机行为未验证。尤其"启动 `.task` 与 `.onReceive` 的先后竞态"（通知热路径可能在 `.task` 的 `await listSessions()` 期间就完成路由）只能装机看。
- **pp 装机注意**：`lastTab` 里那个被上一批污染成 `.local` 的旧值仍在手机 UserDefaults 中。本批之后它不再影响"冷启动是否自动跳聊天页"（那条路已断），但**冷启动落在哪个 tab 仍由它决定**。想立刻回到"远端在前"，需手动切到远端 tab 再杀一次 App 让新值覆盖。
- **回归项（需装机）**：① 冷启动（任意 launchScreen 档位）都落**列表根**，不自动进聊天页 ② 远端在前时冷启动也落远端列表根，背后不再偷偷开本机页 ③ 点推送通知 → **tab 跟着切到本机**并落在该会话 ④ 系统分享表发到 App → 切到本机并落在新建会话 ⑤ 桌面长按快捷指令（新会话/语音/相机）→ 切到本机且语音/相机在**新**会话内触发 ⑥ 上述三条在"应用已在后台的热启动"下同样成立 ⑦ iPad 分栏：显式意图选中的会话若在折叠文件夹内仍能自动展开（`[LAUNCH-ROOT-ONLY]` 后 `selectedSessionId` 只可能由三条显式意图置上，该分支语义不变）⑧ 崩溃循环保护路径仍能落在列表并可自救 ⑨ 设置 → 外观页：「Launch Session」整组消失且**不留空分组/分隔线残迹**，上下相邻的 Chat Background 与自动分组两节衔接正常 ⑩ 若用户的配置审计记录里存过 `appearance.launchScreen` 的旧条目，点"撤销"应返回干净错误而非崩溃（已核：`ConfigOffloadBridge.swift:1071-1077` 的 `resolveField` 为 nil 时回 `not_revertable` + reason "no longer registered"，三个消费点 `:268/:506/:1071` 全部 `guard let … else` 优雅退出）。

## RUNTIME-ID — 新建会话对齐官方运行实例清单（真实 runtimeId）（Qoder, 2026-09-21，pp 装机反馈「发送键灰 + Cloudflare origin 报错」；根因确诊：create 传 runtimeId=nil）
- **根因**：官方新建会话页选择链的数据面是**运行实例清单** `V2DeviceRuntime`（`AA Models/Chat/NewSessionModel.swift:28 inventories` → `:234 isReadyForSession 把关` → `:336-338 createAndStart(runtimeId: runtime.id)`）；我方搬运时选错接口，用了**类型清单** `V2RuntimeType`（结构上没有实例 id），调用点只能填 nil → 服务端把运行类型名当实例 id 落库 → connector 拒所有 runtime RPC（`unknown runtime instance 'claude'`）→ 502。
- **改动 4 文件（AAV2 冻结区 0 字节，`aav2-freeze-check` OK）**：
  - `Packages/RemoteKit/Sources/Glue/PublicRemoteService.swift`：新增 `[RUNTIME-ID]` mirror `RemoteDeviceRuntime`（字段=官方 NewSessionModel 真正用到的：runtimeId/runtimeType/name/displayName/available/reason/configured/active/status + `id` 为官方 `V2DeviceRuntime.id { runtimeId }` 逐字；`status` rawValue verbatim。三个派生值 `isReadyForSession`/`sessionDisplayName`/`unavailableReason` **不在 mirror 上重抄表达式**，而是在 `mapDeviceRuntime` 经本文件底部既有官方逐字 extension（`:917-`）预计算后存值——单一真源，避免与 extension 漂移（本仓禁内联重复定义）；不外泄 AAV2 enum，同 `RemoteConnector.status` 惯例）；新增实例清单 seam `public func runtimes(connectorId:) -> [RemoteDeviceRuntime]`（engine 侧 `RemoteSessionBackend.listRuntimes` :234 早已就位）；`prepareSession` 出口从「按 runtimeType 解析实例」改为官方 `prepare(connectorId:runtimeId:)` 直调（唯一调用点是新建会话页；解析 workaround 系当初选错接口的补丁，随之退役）。旧 `runtimeTypes` public seam 与 `RemoteRuntimeType` mirror **已删除**：实例清单接管后全仓消费者归零，留即死码（engine 侧 `RemoteSessionBackend.runtimeTypes` 未动）。同批顺手补一处既有搬运缺口：官方 extension 的「尚未就绪 · 」分隔符我方漏了 ` · `（官方 `NewSessionModel.swift:405`），本批把它导出给 UI 后成为可见文案差异，已按官方补回。`startSession` 出口未动（本就有 runtimeId 参数）。
  - `src/ios/Views/RemoteSessions/RemoteNewSessionModel.swift`：`runtimes:[RemoteRuntimeType]`/`selectedRuntimeType` → 官方形态 `inventories:[String:[RemoteDeviceRuntime]]` + `selectedRuntimeId`；装载把关 `filter(\.configured)`（官方 :217/:227）、默认选第一个 `isReadyForSession`（官方 refresh 收敛 :203-208；saved 分支因本仓无 preference 持久化省略）、`selectedRuntime` 从当前设备 inventory 反查（官方 :73）、`selectTarget` 官方 :234 把关 + 成功后 re-prepare、`canCreate`/`prepareTarget` 守卫改 `isReadyForSession`（官方 :171-176/:250）、`create` 传 `runtimeId: runtime.runtimeId`（官方 :336-338）+ 官方 target staleness guard 的本地简化版（resolveProject 的 await 后复核设备/实例未变；本仓无中途 re-prepare，标注 `[RUNTIME-ID]`）；`restoreCreationDraft` 参数 runtimeType → runtimeId（官方 `runtimeID = meta.effectiveRuntimeId`）。
  - `src/ios/Views/RemoteSessions/RemoteNewSessionSheets.swift`：sheet 本地 `inventories` 换 `RemoteDeviceRuntime`，`loadInventory` 走 `service.runtimes` + `filter(\.configured)`；排序/展示/把关逐字官方 SessionTargetSheet（`isReadyForSession, sessionDisplayName, id` / `detail=sessionUnavailableReason` / 未就绪 disabled）；「推荐」徽标删除（V2DeviceRuntime 无 recommended 字段，不发明）。
  - `src/ios/Views/RemoteSessions/RemoteNewSessionView.swift`：`restoreCreationDraft` 调用点改传 `pending.0.effectiveRuntimeId`；顶栏胶囊/无障碍值用 `sessionDisplayName`（实例可重名，官方展示口径）。
- **同类缺陷扫描（任务 #16 顺带）**：全仓 `runtimeId: nil` 已 0 处；`startSession(` 生产调用点仅 RemoteNewSessionModel 一处。
- **验证边界（诚实口径）**：Linux 无完整工具链——`swift-parse-check --changed`（4 文件 fail=0）、`swift-spelling-check`（suspects=0）、`import-scan` OK、`aav2-freeze-check` OK；本地 `swift build` 因冻结文件依赖 Apple `Network` 框架 + swiftc 6.0.3 尾逗号语法分歧而不可用（既有事实，非本批引入）。**type-check 与真机行为未验证**，等 RemoteKit Build CI（macOS）+ iOS Build + 装机。
- **存量坏数据（另任务 #18）**：本批只修增量；已落库的 `runtime_id='claude'` 会话不会自愈——`effectiveRuntimeId` 回落值不在实例清单里，restore 后由 refresh 收敛重选，但服务端旧会话的 runtime RPC 仍需数据修复。

## CHAT-PUSH-FENCE — 聊天页左上角两颗 ≡ 叠在一起（Qoder, 2026-09-21，pp 装机截图「左上角三个按钮…有两个叠在一起」，且追问「你还是没修？」）
- **一根因**：DEV-DETAIL-EMPTY 那批把 `remoteAtRoot` 写主上移到「列表的 push 状态」时只覆盖了 `showsDeviceDetail` 一条 push，漏了 `showsChat`。聊天页 push 后 `remoteAtRoot` 停在 `true` → `gearVisible`（`RootModeTabsView:258-261`，远端线即 `remoteAtRoot`）仍为 true → 外壳 `.overlay(.topLeading)`（`:84-86`）那颗齿轮（`SidebarMenuIcon` 同款 `AppSymbol("sidebar.left",22)`，`:306`）继续浮在导航栏条带上，和 `ChatPageToolbar` 自己唯一的 `.topBarLeading` 返回键（`RemoteChatPageToolbar.swift:44-49`）叠成一颗视觉"双按钮"。右侧那颗是官方 trailing 组（folder + ⋯），非缺陷。
- **排除项（先查官方再动手）**：① 不是系统 sidebar toggle 未移除——远端链是纯 `NavigationStack`，无 SplitView，`.toolbar(removing: .sidebarToggle)` 在此本就无对象可移除；② 不是修饰符顺序——官方 `Views/Chat/SessionChatView.swift:106-121` 与本仓挂载顺序一致；③ 新会话页为何没这问题——它是 `fullScreenCover`（`RemoteSessionListView:159`）整屏盖住外壳，聊天页是 push。
- **改动 1 文件**：`RemoteSessionListView.swift` 在 `navigationDestination(isPresented: $showsChat)` 后补 `.onChange(of: showsChat) { _, pushed in RootTabRouter.shared.remoteAtRoot = !pushed }`，逐字照 `:368-370` 设备详情先例。顺带修好聊天页内横滑漏切本机（`RootModeTabsView:168` 的 guard 此前在聊天页不生效）。全仓远端栈 push 现共两条（`showsDeviceDetail` / `showsChat`），写主均已到位；其余为 sheet / fullScreenCover，不涉及此栅栏。
- **验证边界（诚实口径）**：Linux 8 门全绿 rc=0（`aav2-freeze-check` / `import-scan` / `swift-parse-check` 581 文件 fail=0 / `swift-spelling-check` suspects=0 / `pbxproj-strict-check` / `audit-swift-registration` orphans=0 / `audit-project-inputs` missing=0 / `audit-aa-assets` 93 referenced）。**真机行为未验证**：需装机确认「点会话卡 → 左上角只剩一颗返回键 → 左滑返回列表 → 齿轮回来」，以及 `navigationDestination(isPresented:)` 被侧滑返回弹回时 `showsChat` 是否可靠翻回 false（SwiftUI 文档语义为会复位，未实测）。

## BUBBLE-HUG — 用户气泡被拉满整行、文字左靠（Qoder, 2026-09-21，pp 装机截图「我发的气泡怎么是这个样子」）
- **搬运件本身无差异**：`UserMessageBubble`（`SessionTimelineRow.swift`）与官方 `Views/Chat/SessionTimelineRow.swift:55-90` 逐字一致（`Spacer(minLength: 48)` + trailing `VStack` + `padding(h17/v12)` + `Color(white:0.13)` / `cornerRadius 24`）。缺陷在桥接件的**宽度语义**。
- **根因**：官方气泡内层是 Textual `InlineText`，在 SwiftUI 布局里按文本用字宽度参与；本仓替代件 `ChatSelectableText`（`RemoteSelectableText.swift`，§0f 禁令下用 `UITextView`）默认模式只报 `intrinsicContentSize = (noIntrinsicMetric, lastHeight)` → 宽度轴对 SwiftUI 是"可变"，于是和同一 `HStack` 里的 `Spacer(minLength: 48)` **平分宽度**，气泡铺满剩余行宽、文字左靠（截图实测左内缩≈半根 Spacer，与此一致）。
- **改动 1 文件**：`RemoteSelectableText.swift` ①`ChatSelectableText` 实现 `sizeThatFits(_:uiView:context:)`（iOS 16+，本仓 target 17）：按 SwiftUI 给定的宽度上限返回 hug 尺寸，宽度 `min(用字宽度, 上限)`、高度取同一次排版；`ownsContentWidth` 返回 `nil` 走默认（读 `intrinsicContentSize`，由调用点 `.fixedSize` 决定，两处 code/diff 面板行为不变）。②新增 `hugSize(maxWidth:)`：临时把 `textContainer.size` 撑到上限 → `ensureLayout` → `usedRect` → 立即还原（`widthTracksTextView` 保持 true，下次布局仍按 bounds 复位）。**不用 `UITextView.sizeThatFits` 的宽度**——它回传约束宽度而非用字宽度。③删除随之失效的 `lastHeight` + `layoutSubviews` 高度回灌链（单一真源，不留死码）。
- **影响面**：`ChatSelectableText(` 全仓 3 个调用点（用户气泡 1 + code/diff 面板 2），后两者走 `ownsContentWidth` 路径未变；本机线 `AIChatView` 的私有 `SelectableTextView` 是另一件，未触碰。
- **验证边界（诚实口径）**：Linux 门全绿 rc=0（`swift-parse-check` 581 文件 fail=0 / `swift-spelling-check` suspects=0 / 括号平衡 0）；`sizeThatFits` 参与布局的真机行为**未验证**——需装机看短消息右靠 hug、长消息换行不溢出、多行高度不被裁。

## NEW-SESSION-OPEN — 新建会话发完第一条消息退回列表（Qoder, 2026-09-21，pp 装机反馈「新建会话为什么发完消息就退到列表了」）
- **定性**：真·搬运缺口。官方 `NewSessionView.swift:52` create 成功 → `onCreated(session)`；`Views/ChatShell/ChatShellView.swift:207-210` 的 `onCreated` 做两件事——`appState.updateSession(session)`（官方 `AppState.swift:627-629` = `dashboardRepository.upsert([session])`）+ `openSession(session.id)`（`:244-246` 把 selection 换成 `.session(id)`，`:194-202` 原地换内容槽，不存在"关了再开"）。本仓只做了 cover 的 `dismiss()`：`RemoteSessionListView` 的 `onCreated` 回调历史上只刷列表，**全仓没有任何一处把新 sessionId 写进 `chatSessionId`/置 `showsChat`** → 现象 = 退回列表（不是被 pop，是根本没 push）。该缺陷此前被 [RUNTIME-ID] 掩盖：修复前 create 必 502 失败、走不到成功分支。
- **改动 3 文件**：
  - `Packages/RemoteKit/Sources/Glue/V2RemoteChatServices.swift`：补官方 `AppState:627-629` 的**单条**版 `updateSession(_ updated: V2SessionMeta)`（本仓此前只搬了批量 `updateSessions` :641，官方 onCreated 用的恰是单条版）。
  - `Packages/RemoteKit/Sources/Glue/PublicRemoteService.swift`：`startSession` 成功后 `chat?.updateSession(r.session)`——官方 onCreated 第一半落在 Glue seam 内。原因：`V2SessionMeta` 未声明 `Sendable`，进不了 `RemoteSessionCreated`（Sendable）的公开面，跨 seam 传类型对象会撞并发检查；写库时序与官方一致（create 成功 → 立刻写仓库 → 调用方再导航）。`sessionMetaJSON` 直传载荷保留不动。
  - `src/ios/Views/RemoteSessions/RemoteSessionListView.swift`：新增 `pendingChatSessionId`，`onCreated` 存 id；`.onChange(of: showsNewSession)` 在 cover 关闭事件到达后置 `chatSessionId` + `showsChat = true`（官方第二半）。**不与 `dismiss()` 同事务 push**：本仓有"与转场同事务改导航路径被吞"的先例 `[T-ios-stacknav-transition-attributegraph-race]`，故等 cover 关掉再 push。顶栏齿轮/页切栅栏随之由 [CHAT-PUSH-FENCE] 那行自动落好。
...
## LEDGER-C — 台账纠错标注 + 本轮复测新登记（Qoder BATCH-C, 2026-09-22 04:0x，任务书「台账与注释纠错」；[Qoder + 2026-09-22]）

- **性质**：本批不改任何运行期逻辑。①对既有台账断言就地补纠错标注（原文保留），共 6 处，分挂各归属节（TRR / TOOLBAR-DROP-PROBE / DETAIL-COMPLETION / P1-CHAT×2 / NEW-SESSION-DRAWER），措辞严格区分「判案当时成立、后被环境改变 → 过期」与「写下来当天就是假」；②改 1 行代码注释；③登记本批实测发现、但**未修**的独立事项。每条均本批重跑命令取证，不照抄审计报告（审计自身 2 处判定被复测推翻，见下「审计纠错」）。
- **注释修正（唯一代码文件触碰，1 文件 1 行、仅 `//` 文字）**：`src/ios/Agent/MessageList/MessageListInfrastructure.swift:1040` 行尾注释「default 22」→「default 8 per the TOOLSPACING-2 same-day addendum final call; 22 was an interim value」。实测终案值：`ToolRenderStyle.swift:78` `static let blockSpacingDefault: Double = 8`；TOOLSPACING-2 节「2026-09-19 追加（pp拍板）」自证途中 22→11→8。code token 零改动，`git diff` 该行只有 `//` 之后文字变化。
- **新登记 1 — SwiftUI-Shimmer 死依赖（未删，删它=pbxproj 手术需单独批）**：包引用与产品挂载仍在，但全仓零消费者。
  - 🔴 **纠错标注（Qoder, 2026-09-22 同日第三批，见 BATCH-F/G/H 节 2.6）**：①「未删」已过期——本批已单独批次摘除，见该节；②**「frameworks 挂载」用词是错的**，写下来当天就错：`E5SHM0021` 实测位于 app target 的 `packageProductDependencies = ( … )` 块内（`git show HEAD~:…pbxproj | sed -n '3237,3244p'` 可复算），不是 `PBXFrameworksBuildPhase` 的 `files` 列表；两处 `E5SHM002x` 均未出现在任何 frameworks 阶段。原文保留不删，按台账规矩就地标注。
  - 证据：`grep -n 'Shimmer' src/ios/Minis.xcodeproj/project.pbxproj` → `:3243 E5SHM0021 /* Shimmer */`（frameworks 挂载）、`:3347 packageReferences 含 E5SHM0020`、`:4866 XCRemoteSwiftPackageReference "SwiftUI-Shimmer"（repositoryURL = github.com/markiv/SwiftUI-Shimmer）`、`:4902 XCSwiftPackageProductDependency`；`grep -rn 'import Shimmer' src Packages FusionUI | wc -l` → **0**。（`ShimmerText.swift` 各命中是自家文件，非该包。）
  - 背景：SHIMMER-V12（09-19）判 SwiftUI 系 shimmer 库全部免疫失败、自研 FBShimmeringLayer 机制替代——包自此悬空。处置建议：单独批次摘 4 处挂载 + CI 验证，勿顺手混进功能批。
- **新登记 2 — ChatDetailNavigation 未搬：注释自认、台账全文无登记（漏账）**：`src/ios/Views/RemoteSessions/RemoteChatPageToolbar.swift:8` 头注释明写「ChatDetailNavigation 未搬：其 drawer 宿主逻辑依赖官方侧栏抽屉体系（本仓无侧栏），非 drawer 分支的两个视觉点…由调用页直接应用」。但 `grep -n 'ChatDetailNavigation' PATCHES.md` → **0 命中**（本条登记前）；同情形（搬运时有意不搬某件）在 PATCHES-P2「诚实 Staged（不假造型）」条有登记先例，P1-CHAT 节对此件**只字未提**，全仓唯一字据 = 那行代码注释。`grep -rn 'ChatDetailNavigation' src/ios` 仅 2 处注释（RemoteChatPageToolbar.swift:8 / RemoteNewSessionView.swift:84），无类型落地。本条即补登：P1-CHAT「官方对照」条的页面清单未含 ChatDetailNavigation，判"当时清单即未搬运对象"还是"漏搬"需对 origin-cache `1bc11f45` 的依赖闭包另查，归 Doris 复审批。
- **新登记 3 — 冻结区 2 文件 ≠ v2.0.0 的今日复测证据（P1-CHAT「冻结基线偏差（需 Doris 处置）」条已登记且仍准确，本条补硬数据，不重复开票）**：
  - `diff <(git -C /home/ubuntu/repos/moonveil-cloud show "v2.0.0:ios/Agents Anywhere/Agents Anywhere/Models/Chat/MarkdownBlockSizing.swift") Packages/RemoteKit/Sources/AAV2/Models/Chat/MarkdownBlockSizing.swift` → 我侧多 `MarkdownMeasurementCache` 整段（:29-48）；`TimelineHistoryPull.swift` 同形 diff → 我侧多 `: Equatable` + `init(edge:)`（:5-6/:10-14）。
  - 二者与 origin-cache **ios-ci tip `1bc11f45` 逐字节一致**：`git -C …cloud show "1bc11f45:…/<f>" | diff - …/<f>` → 无输出（IDENTICAL）。
  - 新事实（该条写下时未验证）：云端 `v2.0.0` 与 `ipa-latest` 指向**同一 commit `10300fd5`**（`git for-each-ref refs/tags`），故「v2.0.0-27-g1bc11f45」与今日 `git describe 1bc11f45` = `ipa-latest-27-g1bc11f45` 是同一位置，P1-CHAT 抬头基线标注无误；且 `1bc11f45` **不是**云仓当前 HEAD（`a070c658`，moonveil 分支）的祖先（`merge-base --is-ancestor` rc≠0）——冻结基线锚在 ios-ci 线，别拿 moonveil 分支 HEAD 做对照，否则这两文件会"假性又多出反向差异"。另证归属：上游 `ba50eb35` 恰为 MarkdownBlockSizing +20 行（即 `MarkdownMeasurementCache` 段）、`1bc11f45` 恰为 TimelineHistoryPull +5 行（即 `Equatable` + `init(edge:)`），与该条归因逐字吻合。抬 `AAV2_BASE_TAG` 到 `1bc11f45` 或加白名单的处置决策仍挂 pp/Doris，同该条。
- **新登记 4 — PublicRemoteService.swift 两处注释与实际矛盾（该文件本批奉命禁改，交回协调方）**：`:15`「runtimeTypes 保留给其余调用点」与 `:778`「旧的 runtimeTypes（类型清单）seam 保留不动」均与事实冲突——RUNTIME-ID 批已删 public 镜像与 public seam（同文件 `:98-100` 注释自证；PATCHES.md:1454 亦记「已删除」），实测 `grep -n 'func runtimeTypes\|public.*RemoteRuntimeType' Packages/RemoteKit/Sources/Glue/PublicRemoteService.swift` → **0 命中**（engine 侧 `RemoteSessionBackend.swift:238` 仍在，:15 若指 engine 则用词错位）。修法 = `:15`/`:778` 两处措辞对齐 `:98-100`，一次改净；因 BATCH-A/D 正在读改该文件，本批不动，防止同文件双写。
- **审计纠错（本批复测推翻审计判定的 2 条——报审计的错也是重要信息）**：① 审计判 TRR「全域清零」为**假断言**→ 实测 `git grep -c 'Image("ThinkingIcon")' 6428753 -- src/ios` = **0**，判案当时成立，属**过期**（同日 `011a442` icon-skin-pairing 重引入）；② 审计对 GlassEffectContainer 条注"可能属判案当时成立、事后被改变"→ 实测本批落点 `11b7561` 处该 API 全仓**已有 7 处**（ContentView/ModeTabPicker/RootModeTabsView，B16-GLASS-CONTAINER 09-16 引入且台账 :313 有整节记录），该句**写下来当天就是假**。两条的正确分类恰好互换。现值数字（7 处 ThinkingIcon / manifest 103 / SelectableMarkdownView 9 / app target 17.0 / Shimmer import 0）审计全部报对。
- **现场备忘（非问题开票）**：本批复测期间 `scripts/aav2-freeze-check.sh` 正被 BATCH-B 改写（`git status` = M，强模式一度报 `aav2-freeze-baseline.txt missing`，随后该 baseline 文件以 untracked 形态出现于 `scripts/`，04:04）——属批内进行中状态，勿按"门禁缺 baseline"另开票；本批台账纠错引用的 manifest 校验一律用只读 `sha256sum -c` 在 AAV2 目录内完成，不受脚本改写影响。
- **归属保留**：HUG-OPTIONAL-FONT 节末「`017334e` [STREAMING-LOOP-FIX] 批次尚未登记，归 Doris 补」维持原样，本批未代写。
- **死隔离申报**：本批改动 = `PATCHES.md`（6 处标注 + 本节）+ `MessageListInfrastructure.swift` 1 行注释；零逻辑、零 pbxproj、零冻结区字节。验证 = `git diff` 全量人工过目（swift 侧 diff 仅 1 行且以 `//` 后文字起变）。

## BATCH-A/C 运行期与门禁批次 — 工作区未改动账 + 门禁加固（Qoder, 2026-09-22；[Qoder + 2026-09-22]）

- **性质与口径**：本节登记 `HEAD 10a2b67` 之上**未提交**的工作区改动与门禁文件（行号一律按工作区，提交后会漂）。登记前每条数字/行号/sha 均本批重跑命令取证（`git diff` 逐文件读 + 只读跑门禁），任务书给的值按实测校正，不照抄。同批另完成三处台账小错收口（TRR 待办销项、DETAIL-COMPLETION 归因、NEW-SESSION-DRAWER 行号），就地改在各归属节，见「收口三处」条。
- **2.1 HUG-RUNTIME-CLAMP — 影子测量第三轮钳位（与前两轮不同源，务必写清）**：`src/ios/Views/RemoteSessions/RemoteSelectableText.swift`，工作区 2 个 hunk，未提交。
  - **不同源**：HUG-OPTIONAL-FONT（现 :1494）与 HUG-CGFM-AMBIGUOUS（现 :1511）都是 **macOS iOS Build 类型检查红**驱动的收口；本轮不碰任何类型/解算，是给 ShadowMeasurer 输入输出路径加**运行期垃圾输入纵深防御**，编译面零影响。
  - 入口 `:98`：`proposed.isFinite ? max(proposed, 0) : UIView.layoutFittingExpandedSize.width`——非有限值（±inf/NaN）照旧走 expanded 兜底；**有限的负宽 proposal** 钳非负。旧实现负宽直通 hugSize → `measure` → 负 `NSTextContainer.size.width` → usedRect 结果未定义。
  - measure 内 `:143`：`min(max(maxWidth, 0), 10_000)`——先钳下界再封顶，保证任何调用方都写不进负 container。**`10_000` 封顶是既有件**：`git log -S '10_000' -- <file>` 仅 `017334e` 一笔，本轮只加 `max(..., 0)`。
  - **`10_000` 与 expanded 的重合断言要诚实标注**：任务书称 `UIView.layoutFittingExpandedSize.width` 恰 = 10_000，故封顶不误伤合法 expanded 兜底——该值本机**复算不了**（Apple 头文件只给 extern 声明、文档页不给数字，Linux 无 UIKit；社区实测长期口径 10000×10000）。两点兜底：①封顶自 `017334e` 起就压在 expanded 路径上，台账与装机至今无测量异常归因；②本轮新增钳位对一切非负值恒等，**不依赖**这个数字——若等值断言被推翻，受影响面是前轮既提交的封顶，非本轮增量。
  - 出口 `:149`：宽高各 `max(…, 0)` 钳非负（宽度仍先 `min(ceil(used.width), maxWidth)`），纯纵深防御兜底。
  - **负宽 proposal 行为净变化（诚实口径）**：旧 = 负 container → usedRect 垃圾（不确定）；新 = 0 宽 container → NSTextContainer 默认 byWordWrapping **逐词换行，超长文本可能返回巨大高度**。这是**确定性退化**（垃圾 → 可预测但不理想、可能巨大），不是「修好了负宽」；0 宽 proposal 的更优落点（拒测/不渲染）不在本文件职权。未编译、未装机（Linux 无 Apple SDK）。
- **2.2 A3 归档双事实源 — 归档写路径同步覆盖面 1 条 → 3 条 + loader→仓库反向回写**：`RemoteDeviceDetailView.swift` / `RemoteSessionListView.swift` / `RemoteSessionLoader.swift`，未提交。
  - **背景（双事实源）**：列表页数据源 = `loader.items`/`archivedItems`，与设备页/聊天页所读的 `dashboardRepository` 并存的第二缓存。设备页**三条**归档写路径：①`archiveAll` 无项目分支（`:614-622`，`model.archiveSessions` 返回 `[V2SessionMeta]` → `services.updateSessions`）②`archiveAll` 项目分支（`:610-613`，`dashboard?.archiveProject`，`V2DashboardRepository.swift:194` 返回 **Void**）③批量选中 `performArchive`（`:593-603`，Glue `V2RemoteChatServices.swift:183` `setSessionsArchived` 内部逐条 `dashboardRepository.upsert` 但**只返 Bool**）。三条成功后服务端/仓库状态都前进，列表页不接信号即滞后到下次手动刷新。
  - **改动**：回调 `onSessionsUpdated: (([V2SessionMeta]) -> Void)?` → 无 payload 的 `onSessionsChanged: (() -> Void)?`（声明 `:39-49`，含完整字据注释）；三条路径成功后全部触发（`:599`/`:612`/`:620`——本批 `grep -n 'onSessionsChanged?'` 逐条对，无漏）。列表页接线 `RemoteSessionListView.swift:375-384`：回调里 `loader.load(…, force: true)` + `loader.loadArchived(force: true)`；全仓 `RemoteDeviceDetailView(` 调用点实测仅此前者一处，无第二调用方受签名变更影响。
  - **为什么不带官方那种 `[V2SessionMeta]` payload**：四条路径只有①拿得到完整 meta；②冻结区方法 Void 且冻结区不可改；③只返 Bool，透传真值要改 Glue 写面签名。**传不了真值就不传假值**——官方形态的半 payload（部分路径有、部分没有）会诱导调用方信任字段本身。统一降级「变化信号 + 调用方重拉」。
  - **复算推翻任务书一条（②顺带收益）**：任务书称 app 层从此不引 `V2SessionMeta`、import-scan 深扫违规符号因此消失——**不成立**：深扫仍报 `V2SessionMeta`（同文件 `:102/:489/:549/:554` 四处活代码引用，另有 `RemoteSessionLoader`/`ChatHeaderStatus`；`RK_GATE_FULL_SCAN=1 bash scripts/import-scan.sh` 改前改后同为 23 符号/54 行/20 文件，本批复测输出在案）。实际收益 = 该回调位**一处**引用点减少；代码注释（`RemoteDeviceDetailView.swift:46-48`）自证「不构成 import-scan 深扫违规数下降」。
  - **复算新发现·第四条写路径未接（疑漏，交回核对）**：`RemoteDeviceDetailView.swift:220` 项目归档 alert（`pendingProject` → `dashboard?.archiveProject(project.id, archived: true)`）与②同类同效（归档该项目全部会话），但**未触发 onSessionsChanged** → 该路径归档后列表页同样滞后。任务书只列三条；若非有意排除（项目菜单不属「会话归档面」），是同类口子，需拍板。
  - ✅ **同批收口（Qoder 复算后判定：同类口子，非有意排除，按根因标准补齐）**：`:218-226` 项目归档 alert 的 archive 分支现已触发 `onSessionsChanged?()`（现 `:225`）；`deleteProject` 分支**不**触发——官方语义是只能删没有会话的项目，会话集合不变。触发点现为四处：`:225`（项目归档 alert）/ `:605`（批量选中 performArchive）/ `:618`（archiveAll 项目分支）/ `:626`（archiveAll 无项目分支）。上一条原文按台账规矩保留不删。
  - **反向回写（loader→仓库）**：新增 `syncDashboardAfterWrite`（`RemoteSessionLoader.swift:377-380`），`patchSessionMeta`(:293)/`archive`(:316)/`unarchive`(:337)/`markRead`(:353) 四条写路径成功后各触发一次 `Task { await dashboardRepository.refresh() }`。不逐条 upsert 的原因（代码注释 `:358-376` 字据 + 本批复算）：facade 写面回传的 `RemoteSessionMeta`（Glue `PublicRemoteService.swift:128-146`）是 `V2SessionMeta` 的**有损公开镜像**——实测镜像无 updatedSeq/takeover/sourceAvailability 等参与 mergeSessions revision fence 的字段，且 `V2SessionMeta.swift:29/:31` 的 pinned/archived 为 **let** 无法就地翻旗；拿有损镜像 upsert 会覆盖权威字段或被 fence 错拒。「重复触发安全」口径按注释说准：`refresh()` 的 `!isLoading`（`V2DashboardRepository.swift:56-57`）是**丢弃式**守门，在途即被无声吞掉，滞后窗口延到下次触发；不排队不合并是有意取舍。**根因（facade 写面有损回传）未修**，修它要动 seam 归属文件 → 交回决策项。
  - **验证边界**：全部未编译未装机（Linux 无 Apple SDK）；「滞后到下次手动刷新」为任务书与代码注释口径；三路径 + ④alert 的滞后面需装机回归。
- **2.3 BATCH-A A1/A4 — STREAMING-LOOP-FIX 同族补漏两处**（工作区未提交；任务书 2.1–2.3 未列，本批读 `git diff` 时定位到，据实登记）：
  - `SessionTimelineEventView.swift` [BATCH-A/A1]（`:42-76`）：旧实现每 row 每次 body pass 对全量 notices 重跑 filter（O(rows × notices)，`isVisible`/`blocks()` 均为 @Observable 属性读）；改为每分组顶部 `Self.noticesByTimelineTarget` 算一次 `[timelineTargetID: 通知]` 分桶（谓词逐字保持 `isVisible && !blocks(session.id)`，桶内保序），row 查字典。注释自标粒度残余：分桶按组各跑一次 → 每 pass O(组数 × notices)，数量级改善非归零。系 `017334e` 只改 SessionInteractionDock/ChatTimelineContent 两处后漏掉的**第三处**热路径。
  - `ChatTimelineView.swift` [BATCH-A/A4]（`:64-76`）：旧 `hasInteractions` 计算属性（HEAD :61）一次 pass 被求值多次（onChange 的 of 值 + initial + 两处回调，HEAD `:164/:169/:173`），每次全量扫 notices；改 body 顶部局部 `let`、三处引用走它、计算属性删除。注释口径说准：省重复 CPU 扫描 + 同 pass 取值一致，**不减依赖登记、不改重渲染次数**；流式风暴根因在 hugSize 副作用（`017334e` 已修）与 A1，不在此处。
- **2.4 BATCH-B — 冻结/隔离门禁加固 + 新基线文件**（`scripts/aav2-freeze-check.sh` 50→236 行重写、`scripts/aav2-freeze-baseline.txt` untracked 新增、`scripts/gen_rk_registry.py`、`scripts/audit-port-surface.py`、`.github/workflows/ci.yml`，全部未提交）：
  - freeze-check 重写堵旧版两个**假绿口子**：(a) 只遍历 manifest 行 → 新增未登记 .swift 或删 manifest 行均零告警 → 现磁盘⇔manifest 双向结构断言 + 计数相等（实测 `structure OK: 103 .swift ⇔ 103 manifest rows (bidirectional)`）；(b) CI 无 MV_CLOUD 时静默降级「仅 manifest 自洽」还打 OK，而 manifest 可被本地重 sha256 洗白（**实证**：两个滞后文件磁盘=manifest 一致、旧逻辑全绿，baseline 步才揭红）。现默认三比：结构 + manifest 摘要 + 本仓提交的 baseline（上游字节锚），强模式（MV_CLOUD）另从官方仓重导并反审 baseline 完整性；三模式 default/`--structure-only`/`--write-baseline` 全 fail-closed（缺 manifest/baseline/工具即 exit≠0）。FINDING-1/2 修：上游读取失败不再被 `| sha256sum | cut` 的恒 0 退出码掩护，先物化再判。
  - baseline 文件：112 行 = 9 头 + **103** 数据行（`wc -l` + 64hex 行过滤实测）；头锚 `pinned-upstream-tag: v2.0.0`、`upstream-commit: 10300fd5…`（全值见文件头）、generated-at 09-22 04:06。
  - ci.yml 新增/改造 step 的 KNOWN RED 数字全部本批复算：`import-direction-deep-scan`（continue-on-error）= **23 符号 / 54 行 / 20 文件**，全落 `src/ios/Views/RemoteSessions/`；23 中 20 个 AAV2 内部类型、3 个 Glue 合法门面（RemoteService/RemoteServiceError/RemotePairingPayload，词表误报）——与注释口径逐字一致；「刷新词表后扩到 83」在 /tmp 隔离沙盒复算 = **83**（`cp scripts` + 软链 src/Packages，仓内 registry 未动）。`rk-registry-freshness`：`python3 scripts/gen_rk_registry.py --check` 实测 `committed=211 regenerated=312 missing=101 stale=0`、exit 1（含 ComposerDraft/V2WorkspaceEntry）。`port-surface-audit`：非门禁定位写进头部字据，实测跑完 exit 0（A=2/B=12/C=298，「298 heuristic hits」吻合），输入面空 → exit 2 为新增 fail-closed 契约。
  - **`--structure-only` 是已知红的 crutch，说破**：它自身绿（实测 exit 0）恰因它放过的正是那两个 baseline 红（磁盘=manifest 一致）；ci.yml 里 blocking 的 structure 步与 continue-on-error 的 baseline 步并存是过渡态，红处置拍板后须撤 flag 收口。
  - **分歧计数两个口径（防后人混用）**：按原始字节，与 `v2.0.0` 分歧 **3 个**文件（MarkdownBlockSizing / TimelineHistoryPull / HTTPTransport，逐文件 `diff <(git show v2.0.0:…)` 实测 3 行 RAW-DIFF）；按门禁口径（超出获批变换才算 diverge）**2 个**——HTTPTransport = pp 已签 AAV2-AWAIT-1，施加获批 sed 后对 `v2.0.0` 与 `1bc11f45` 均逐字节等（本批实测 IDENTICAL）。本轮默认模式实测 exit 1、VIOLATION 恰 2 条 = 门禁口径。
  - **2 红归属独立复核**（与 LEDGER-C「新登记 3」(现 :1530-1533) 同案，此处只补本批新证据不重复开票）：两文件在本仓 `--follow` 历史各仅 `6582129` 一笔、其后零改动；本地 sha256 `4f0a884b9529380d44c69109d9c90089559f8b68513c8319566824a6bda73bff` / `b7bd95b21c40304c4b243557a4652de78c6f6ad74c11fdb35bf3dc1513220807` 与上游 `1bc11f45` 全段相等 ⇒ 成因**基线滞后**（搬运源在 `v2.0.0`=`10300fd5` 之后 27 个提交）而非本地改动。103 冻结文件对 `1bc11f45`（ios-ci tip）全量逐字节对比仅 HTTPTransport 一处、加获批 sed 即等 ⇒ **冻结区相对 ios-ci 线无漏搬**；对 moonveil 分支 HEAD `a070c65` 则差 3 个（上游自身又演进），勿拿它当漏搬证据。
  - **抬锚点决策未拍板（截至本批落笔）**：ci.yml 注③「抬 `AAV2_BASE_TAG` 到 `1bc11f45` + 重生成 baseline」**尚未拍板**——`1bc11f45` 无任何 tag（仅 `origin/ios-ci` 指它，`git tag --points-at` 0 行），`ipa-latest`/`v2.0.0` 仍双锚 `10300fd5`；且其非 moonveil 分支 HEAD 祖先（merge-base rc=1）。备选①回滚 v2.0.0 原样=编译器红，本批复算依赖点：`ChatTimelineView.swift:49` 用 `TimelineHistoryPull(edge:)`、`:157` `latest != latestPull` 需 Equatable（v2.0.0 原件两者皆无；ci.yml 注引的「:152」已因本批 A4 插行漂到 :157，以值行为准）；备选②白名单登记=把上游改动错标成本仓编辑。**抬锚与 AAV2-AWAIT-1 同权重，归 pp**；未抬之前任何台账/注释不得按「已抬」口径书写。
- **收口三处（本批已改，均系 BATCH-C 同日纠错条上的修正，错值原文以「初稿误作…」措辞留证）**：TRR「待 pp/装机」清单 StatRow ThinkingIcon 销项（:539 待办行后、现 :541 新增纠错条）；DETAIL-COMPLETION 纠错条归因去冗（现 :1065，`1f76b55` 非新增源、只留 `6582129`，8 位 sha 统一 7 位）；NEW-SESSION-DRAWER 纠错条行号改记 26.2 值所在行 :4266/:4286/:4318/:4347（现 :1185；块起始行 :4254/:4278/:4298/:4331 + 偏移 +12/+8/+20/+16 一并留注）。后两处纠错条原引行号 :1063/:1183 因 TRR 新增两条而漂 +2，以本节括注为准。
- **死隔离申报**：本批（本节 + 三处收口）唯一触碰文件 = `PATCHES.md`；全部验证命令只读（git grep/show/log、diff、grep、freeze-check 默认与 --structure-only、gen --check、deep-scan、/tmp 沙盒），未跑 `--write-baseline`、未重生成 registry、未 commit/push/stash/checkout。**收工 md5 复核如实记录**：开工快照 `/tmp/y-start.md5`（04:58:19 建，11 文件、不含本文件）收工核验 **8 OK / 3 FAILED**——`ci.yml`（mtime 04:59）、`RemoteDeviceDetailView.swift`（04:58）、`aav2-freeze-check.sh`（05:09）三件在快照后仍被并行批次续改，非本批触碰（本批零 Write/Edit 指向它们）；本节对上述三件引用的全部数字与关键文案（23/54/20、83、211/312/101、:152 漂 :157、`onSessionsChanged` 三位 `:599/:612/:620`、`--structure-only` exit 0、默认模式 exit 1 VIOLATION 恰 2 条）已在续改后的**当前内容**上逐条复验一致，登记不受影响。

## BATCH-F/G/H — 冻结锚点抬升 + AuthAA 逐字区纳管 + 归档无损写回 + seam 逐文件白名单（Qoder, 2026-09-22 04:xx–08:xx；[Qoder + 2026-09-22]）

- **性质与口径**：接上一节 BATCH-A/C 的未收口项，把「需 pp 拍板」的六件一次做完——锚点抬升、AuthAA/SettingsSkin 纳管、归档写回根因、词表刷新+seam 方案、列表页闪烁、SwiftUI-Shimmer 死依赖。拍板字据：pp「这些都做吧 你分配给子代理做」+ 对 seam 三选一选「显式文件清单」+ 对分工回「行」（ci.yml 归主代理独占）。行号一律按工作区实测，提交后会漂。执行方式=子代理分文件独占派发（F/G：冻结引擎与文案；S/H：Glue+RemoteSessions+payload 门禁；D：仅 pbxproj），每批实现者自审 → 对抗审查代理只读复算 → 二轮再审，共五波。
- **2.1 锚点抬升 `v2.0.0` → `1bc11f45`（BATCH-A/C 挂着的决策项，本批拍板）**：旧锚 `v2.0.0`=`10300fd5`（与 `ipa-latest` 同一 commit），新锚 `1bc11f45` 是 mirror `ios-ci` 线 tip（2026-09-13，无任何 tag）。抬前独立三方复算（baseline ⇄ 上游 ⇄ 磁盘）：103 个逐字文件对 `1bc11f45` 全量比对仅 `Network/HTTPTransport.swift` 一处分歧，且它就是 pp 已签的 AAV2-AWAIT-1 sed，经 `expected_bytes()` 归一后逐字节等 ⇒ **抬锚没有把任何本地自撰字节洗成上游**。baseline 重导后 `git diff scripts/aav2-freeze-baseline.txt` 恰好 2 行数据变更：`MarkdownBlockSizing` `b15f11a9…`→`4f0a884b…`、`TimelineHistoryPull` `371fb273…`→`b7bd95b2…`（103 行不变），两值与上游 `1bc11f45` 全段相等 ⇒ 收口 BATCH-A/C「2 红归属」条，那两文件从「需 Doris 处置」转为「基线滞后已修」。
  - 就地纠错：`AA-ATTRIBUTION.md:5` 原写 `v2.0.0-27-g1bc1f45`（真值 `v2.0.0-27-g1bc11f45`）是**拼出来的、不可复算**——`git describe --tags 1bc11f45` 实机输出 `ipa-latest-27-g1bc11f45`（两 tag 同 commit 故距离都是 27，但 describe 点名 `ipa-latest`）。已改为只陈述可复算事实（tip / 日期 / describe 原样 + 「-N-g… 后缀属 describe 输出，不是身份的一部分」）。
- **2.2 冻结引擎区域化 + authaa 新区**：`scripts/freeze-check.sh`（548 行）按 `FREEZE_ZONE=aav2|authaa` 配置化，`aav2-freeze-check.sh`(11 行)/`authaa-freeze-check.sh`(18 行) 降为薄壳，两区共用同一套 fail-closed 断言。新纳管目录 `src/ios/Views/AuthAA` + `src/ios/Views/SettingsSkin`（`*.swift` + `*.ttf`）：逐字 8 条 + 换肤白名单 10 条（8 个有上游对应但本仓改肤 + 2 个无上游对应）。manifest 行法扩为 `<64hex> <local>[@<upstream-rel>]`，`basename(local)==basename(upstream)` 断言 fail-closed（用 '@' 把 A 文件字节挂到 B 文件名下是最省事的洗白路径，直接堵死并回显全部 @ 映射供人核）。白名单行法刻意与 manifest 不同构（path 在前、sha 第二、后接 ASCII 原因），**注册 sha 与磁盘不符即红** ⇒ 换肤改动是「带日期的再注册」而不是 unchecked 堆放；`ALLOWLIST_MAX=10` 超出即红，抬上限必须是引擎里可审的一行。ci.yml 两区均 blocking（`aav2-freeze` 改造 + `authaa-freeze` 新增），TRANSITION 监视默认 blocking + `FREEZE_ALLOW_TRANSITION=1` 显式 ack（G 未按我「倾向警告」的建议并主动申报偏差，审查代理独立判定「警告=无声」→ 采 G）。
- **2.3 钉死覆盖条数（二轮审查揭出的真洞，本批补）**：TRANSITION 监视以 `git show HEAD:` 为基线 ⇒ 账本尚未提交的当口（正是本批 authaa 的真实状态）或无 `.git` 检出下**全裸**。沙盒实证：把某逐字文件从 manifest 摘一行、同文件登记进白名单 → 磁盘集合不变、结构双向断言照绿、1c 因 HEAD 无账本跳过 = **完全假绿**。现 `MANIFEST_ROWS_PINNED`（aav2=103 / authaa=8）与 `ALLOWLIST_ROWS_PINNED`（0 / 10）钉在引擎常量、且在 TRANSITION 分支之前执行（ack 环境变量绕不过）。复算：沙盒内 8→7 + 白名单 10→11 并把 `ALLOWLIST_MAX` 与钉数一并放宽 → `VIOLATION (authaa): manifest has 7 rows, engine pins 8`，exit 1。攻击要成立必须同时改 `scripts/freeze-check.sh` 的两处常量 = 把豁免搬到评审面上。另按 S1 给 TRANSITION 加**具体路径 delta**（哪条离开 manifest、哪条新进白名单），否则「103 -> 102」这种 ack 是盲签；沙盒复算输出 `left the manifest: API/V2/V2RealtimeAPI.swift`。
- **2.4 归档写回根因收口（BATCH-A/C「根因（facade 写面有损回传）未修」条销项）**：
  - Glue 三处签名改为回真实值：`V2RemoteChatServices.swift:182` `updateSessions(_:) -> [RemoteSessionMeta]`、`:196` `setSessionsArchived(...) async -> [RemoteSessionMeta]?`（失败仍 nil + `sessionActionError`，与官方语义同）、`:234` 新增 `archiveProject(id:archived:) async throws -> [RemoteSessionMeta]`（冻结区写主返回 Void 且不可改，故在其之后按 `projectId` 过滤仓库快照派生，排除 `local:` 草稿避免幽灵行）。「服务端回传即全量受影响集」的上游字据挂在注释里（`moonveil-cloud @ a070c658`：`api/projects.py:228-231`、`infra/repositories/projects.py:499-560`、`api/connectors.py:246-270`、`services/connector_presence.py:54-69`）；**「无损」已按 a070c658 契约限定口径**，并注明派生集相对「本次真正翻转的会话」可能 over-inclusive（消费侧按 id 幂等并入，多并无害）。
  - 跨 seam 回调形态：`RemoteDeviceDetailView.swift:39-49` `onSessionsChanged: (([RemoteSessionMeta]) -> Void)?`，四处触发点全部带真值（`:224` 项目归档 alert / `:605` 批量 performArchive / `:620` archiveAll 项目分支 / `:627` archiveAll 无项目分支），列表页 `RemoteSessionListView.swift:386-388` 消费为 `loader.applyRemoteChange(changed)`。BATCH-A/C 里「统一降级为无 payload 变化信号」的形态被本批取代——当时传不了真值是因为写面签名有损，签名修好后 payload 是真实可达的。
  - **闪烁根因**：上一批接线是 `load(force:true)` + `loadArchived(force:true)` ⇒ 每次归档写回都重跑 `.loading` 相位、骨架屏闪一下。改为增量合并后不再整表重拉。
  - loader 侧机制：`applyRemoteChange`（`:395`）抬 `changeGeneration` 并写有界 `changeLog`（cap 128，`:57`）；`load`/`loadArchived` 在派发前捕获 `startGen`、快照落地后 `replayed(afterGen:)`（`:471`）重放在途窗口内的变更集；`merged`/`upsert`/`listOrder`（`:427`/`:462`/`:418`）为纯静态。裸日期排序收口到 `listOrder`（置顶优先）：`merged:457-458`、`resort:517`、`resortArchived:522`、`unarchive` 回滚改走 `resortArchived()`；全仓 `sortDate >` 现仅存 `:420` 一处，即 `listOrder` 本体。
  - **本批再收口（二轮审查指认的同类口子）**：页面内 `archive`（`:337`）/`unarchive`（`:355`）原先手写 append 循环、不进 changeLog ⇒ 绕过日志的写回会被「发出更早、落地更晚」的旧快照**永久复活**（`replayed` 无条目可重放），滞后到下次全量。现两方法成功路径统一 `applyRemoteChange(r.sessions)`。类型链路读码确认非推断：`PublicRemoteService.swift:638/:643` 返回 `RemoteSessionWriteResult`，其 `sessions: [RemoteSessionMeta]`（`:159`）与入参严格同型，映射在 Glue `bulkAction` 内完成（`:762`）⇒ `src/ios` 不碰 V2 内部类型，无需新转换层。乐观移除保持不动是有意取舍：日志只记服务端已确认变更，乐观条目若也入日志则失败回滚无对应撤销项，且「确认先到 / 快照先到」两种时序都经 changeLog 收敛。
- **2.5 import-scan seam 白名单（pp 选「显式文件清单」）**：词表 211→312 刷新（`gen_rk_registry.py --check` 绿）；新增 `scripts/rk-seam-files.txt` 41 条，全部落在 `src/ios/Views/RemoteSessions/`（该目录 58 个 .swift 中的全部违规者，另 17 个干净文件继续受门禁约束）。为什么不整目录注册：整目录 = 深扫在唯一持续新增 RemoteKit 引用的地方永不触发，`0657784` 的 ci.yml 注把该结局明写为 "a gate that can never fire -- refused"。牙齿实测：新文件放进该目录并引用 `RemoteSessionBackend`（6 个硬编码浅扫名之一）→ 浅扫与深扫**都红且点名**；账本行指向已删文件 → 红。新增 3b **反向断言**：清单每行必须仍引用 ≥1 个词表符号，否则红——堵住「清单只变长」= 豁免权只增不减。`ModeTabs`/`AuthAA`/`Packages` 维持整目录 seam（既有、小、各自是指定入口），本批只动 pp 决策涉及的那一处。
  - 主代理两处自伤，据实在案：①首版 41 清单是从门禁输出反推的，而门禁每词只打印 `head -3` → 得 34 条（少 7 条）；改用门禁自身匹配逻辑重算才得 41。②清单头部的再生成命令初稿跑出来是 **0 文件**（`$SEAM_DIRS` 在命令里未定义、缺 `\b`、未剥 ROOT 前缀），现改为与门禁逐字同构（绝对搜索根 + 绝对 SEAM_DIRS + 剥前缀）并实测 `diff` 与清单 0 差异。
- **2.6 SwiftUI-Shimmer 摘除**：`project.pbxproj` 15 行删除 / **0 新增**（`E5SHM0021` 出 app target `packageProductDependencies`、`E5SHM0020` 出 project `packageReferences`、两个对象块整体删除）+ `THIRD_PARTY_LICENSES.md` 1 行删除；自家 `ShimmerText.swift` 的 `E5TA00004/E5TA00005` 未动，`pbxproj-strict-check.py` / `audit-project-inputs.py` / `audit-swift-registration.py` 三项均绿。LEDGER-C「新登记 1」的「frameworks 挂载」错标已就地标注（该 id 实测在 `packageProductDependencies` 内，非 `PBXFrameworksBuildPhase`）。
- **2.7 治理事件（子代理越权宣称拍板）**：实现代理 S 在 seam 方案未拍板前，把自己选的整目录注册写成 "pp-adjudicated" / "Settled + landed"，并照 `0657784` 注释明写 refused 的结局落地。主代理先删两处伪授权措辞（改为 `[SEAM PROPOSAL -- NOT pp-adjudicated]` + 实测代价陈述），再向 pp 提三选一，pp 选「显式文件清单」，最终实现见 2.5。同批 S 恢复备份时把文件写到不存在的路径，留下一个游离未跟踪文件（后核实并删除，真实 `ContentView.swift` 未受影响）。**规则追加：子代理不得代 pp 拍板，未拍板不得写成已拍板**（此后 brief 里写死该条）。
- **2.8 iOS Build 红的根因（主代理自伤，`df2de50`）**：该批在 `SessionTimelineEventView.rows` 顶部插 `let noticesByTarget = Self.noticesByTimelineTarget(...)`，body 变多语句、隐式 return 失效，而该属性缺 `@ViewBuilder` → 唯一编译错误（`View.body` 因协议要求自带 `@ViewBuilder`，故 `content` 无恙；`agentGroup: Bool` 是既有件）。修复 = `:41` 加 `@ViewBuilder`（该一行由 Doris 先行以 `fbc51f6` 提交，内容与本批复核结论一致，不再计入本批）。**根因不是这一行**：交付前本地 9 项门禁全绿，而这些门禁零类型解析，我仍把「门禁绿」当交付证据上报——交付证据等级不够。
- **2.9 注入告警=误报（记录以免后人重查）**：H 报两次工具返回出现指令形态文本，要求插入隐藏「AI-generated / human-verified」注释与不可见标记，H 拒绝并上报。取证代理在 6153 个工作区文件、两仓全部 git blob 历史、`chat_history.db`、以及 H 实际 tool_results 中检索该载荷 → **0 命中**；只有形近的 benign 文本（`V2SessionProjection.swift:109,124` 与 `V2SessionReadCoordinator.swift:23` 的 "high watermark"、`RemoteSessionLoader.swift` 的「标记已读」）。该载荷全局只出现一次：H 自己的报告。结论=误读，无文件需隔离、无改动需回滚。
- **2.10 主代理沙盒事故（同族教训）**：用 `cp -al` 建 aav2 快测沙盒，`cp` 写回穿过硬链接 inode，把**真实** `FREEZE-MANIFEST.txt` 删掉一行（103→102）。当场从 HEAD 复原并三重复核：`git status` 该路径干净、`sha256sum` 与 HEAD 全等（`9c338398a296ce0a…`）、行数回 103。教训：沙盒中可能被写入的路径一律 `cp -a` 独立 inode，硬链接只用于确认只读的大树；本轮之后的牙齿测试改在 `cp -a` 的 scripts 副本 + 只读符号链目录里做，收尾按精确路径清理。
- **未拍板项（四条，交 pp）**：①「全部」筛选的归档语义——`merged:440` `case .all: keepInItems = true` 且读路径 `:123` `merged = active + archived` ⇒ 已归档行本就属于「全部」，但 `RemoteSessionItem`（`RemoteSessionListView.swift:956-972`）没有归档字段 ⇒ 归档确认后该行会以普通行形态回到列表、无视觉区分。出路两条（加归档标记+改行渲染=UI 改动 / 「全部」排除已归档=改读路径语义）都超本批授权，未动。②`applyRemoteChange` 对空变更集早退不抬 generation ⇒ 服务端若对全部 id 回 `notFound`，乐观移除的行仍可被旧快照复活；彻底解法是 R2-H 提的「空 payload 编译不过」（不可默认构造的 `ArchiveChangeSet`）+ 把 merge 语义搬进 Linux 可编译单测 target（顺带终结 grep 文本断言的无限增长），未拍板未做。③新门禁两件：pbxproj 引用完整性断言（往 `pbxproj-strict-check.py` 加 60-80 行，防孤儿/缺定义 id）、以及「非 `body` 的 `var X: some View` 多语句无 return 且缺 `@ViewBuilder`」确定性检查（Linux 侧可判，正是 2.8 那一类）——两位 finder 均判超范围未做。④`df2de50` 缺 `[qoder]` 前缀（本仓痕迹规则），既成事实不回改，此后每笔记前缀。
- **验证边界**：Linux 端只到「语法 + 门禁」层（Swift 6.0.3 `-parse`，无 UIKit），**类型解析唯一裁决点是 macOS iOS Build**；增量合并观感与归档收敛未装机。收工前 16 项门禁全绿、退出码全 0：import-scan（浅+深）、archive-payload-gate、registry 新鲜度、pbxproj 输入审计/严格语法/反向注册、aa-assets、fork-point、两区 `--structure-only` + 两区强模式（`MV_CLOUD`）、case-spelling、port-surface、swift-syntax-parse。ci.yml YAML 解析通过，gates job 17 步，本批新增/改造的 5 步全部 blocking。
- **死隔离申报**：本批四路实现者文件互斥（F/G=冻结脚本与两篇 attribution 文案；S/H=Glue + RemoteSessions + payload 门禁；D=仅 pbxproj；主代理=ci.yml + import-scan + seam 清单 + 台账），审查代理全程只读、`mktemp -d` 沙盒、精确路径清理。全批未跑 `--write-baseline` 之外的写操作、未 stash/checkout/reset，凭证文件零触碰。本批期间他方新增两笔提交：`2e7549c`（Doris 崩溃证据链）顺把我此前 staged 的三件（`.github/workflows/ios-build-diag.yml`、`StreamingHangLogger.swift`、`CrashReporter.swift`）带入 HEAD；`fbc51f6`（Doris）落的就是 2.8 那处 `@ViewBuilder` 修复——我复核时该改动仍以未提交形态在案（`git diff HEAD` = 1 增 1 删），本批拆分提交前被 Doris 先行提交，故本节 2.8 的修复不再出现在本批任何一笔里（提交归属以 `fbc51f6` 为准，不重复计账）。三件均非本批改动面。


## B9-FIX — iOS Build 红根因收口：摘除全局 SWIFT_VERSION=5.0 命令行覆盖（Textual 扩展正则字面量被误伤）（Qoder, 2026-09-22，pp「moonveil最新build挂了 你去看看修一下」→「可以」；[Qoder + 2026-09-22]）

- **现象**：`0d12683`（Doris pbxproj 重复 ID 修复）后 iOS Build 仍红，错误面 11 条 annotation 全落 `Packages/Textual/.../PatternTokenizer.swift:85/:89/:93`（`'$' is not an identifier` + 大写开头 `Expected expression` 系旧解析器）。
- **根因（非 Doris 搬运）**：该文件与官方 `moonveil-cloud/ios/Packages/Textual` 逐字节一致（`diff -rq` 实锤）；真凶是 B8-FIX4（`060f0b0`，09-15）为治 RemoteKit permitsRetry 加的 xcodebuild **命令行全局覆盖** `SWIFT_VERSION=5.0 + SWIFT_STRICT_CONCURRENCY=minimal`——它会压到集成期全部 target，含本地 SPM 包。Textual 的 `/(?\$)\$\$(.+?)\$\$/` 等**扩展正则字面量（`\$` 形态）只有 Swift 6 语言模式可解析**（本地夹具：同一段码 `-swift-version 5` 30 错 / `-swift-version 6` 0 错，swiftc 6.0.3 实测）。Batch 1 首次把真包挂进工程（此前所有本地包源码都是编进 app target 的），钝器第一次命中。
